...
for %%f in ( gsoap condor_util_lib condor_cpp_util condor_io condor_classad condor_procapi condor_sysapi condor_daemon_core condor_acct condor_qmgmt condor_collector  condor_schedd condor_config_val condor_dagman condor_findhost condor_history condor_mail condor_master condor_negotiator condor_preen condor_prio condor_q condor_qedit condor_submit_dag condor_rm condor_shadow condor_startd condor_starter condor_gridmanager condor_stats condor_advertise condor_fetchlog condor_status condor_submit condor_tool condor_version condor_wait condor_userlog condor_userprio condor_store_cred condor_birdwatcher) do ( awk "{if ($0 !~ /microsoft sdk/) print }" %%f.dep > %%f.dep.tmp && del %%f.dep & ren %%f.dep.tmp %%f.dep & echo . & echo *** Building %%f & echo . & nmake /C /f %%f.mak RECURSE="0" CFG="%%f - Win32 %conf%" %* || goto failure )
echo .
