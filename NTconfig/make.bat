...
for %%f in ( gsoap condor_util_lib condor_cpp_util condor_io condor_classad
condor_kbdd_dll condor_procapi condor_sysapi condor_daemon_core condor_acct
condor_qmgmt condor_collector condor_config_val condor_dagman condor_findhost
condor_history condor_mail condor_master condor_negotiator condor_preen
condor_prio condor_q condor_qedit condor_submit_dag condor_rm condor_schedd
condor_shadow condor_startd condor_quill condor_had condor_replication
condor_transferer condor_starter condor_drmaa condor_gridmanager condor_stats condor_advertise condor_fetchlog condor_status condor_submit condor_tool condor_version condor_wait condor_userlog condor_userprio condor_store_cred condor_cod condor_transfer_data condor_birdwatcher) do ( awk "{if (($0 !~ /microsoft/) && ($0 !~ /externals/)) print }" %%f.dep > %%f.dep.tmp && del %%f.dep & ren %%f.dep.tmp %%f.dep & echo *** Building %%f & echo . & nmake /U /C /f %%f.mak RECURSE="0" CFG="%%f - Win32 %conf%" %* || goto failure )
echo .
