...
JICShadow::initMatchSecuritySession()
{
	...
	if( rc < 0 ) {
		dprintf(D_ALWAYS, "SEC_SESSION_FROM_MATCH: failed to get security "
	...
	if( m_reconnect_sec_session ) {
		...
		if( !rc ) {
			dprintf(D_ALWAYS, "SEC_SESSION_FROM_MATCH: failed to create "
	...
	if( filetrans_session_id.Length() ) {
		...
		if( !rc ) {
			dprintf(D_ALWAYS, "SEC_SESSION_FROM_MATCH: failed to create file "
