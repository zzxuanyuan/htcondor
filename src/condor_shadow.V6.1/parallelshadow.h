 ...
 private:
    ...
    int startComrade(int cmd, Stream *s);
        ...
        /** Does necessary things to the args obtained from the 
            sneaky rsh. */
    int hackComradeAd( ClassAd *ad );
  
  
        /** Pretty simple: tweak envs including path to give 
		    information to invoke fake rsh
		*/
    int hackMasterAd( ClassAd *ad );

        /** Start a invoking process that is specified by the ad.
		 */
    ...
    /**********************
	     for Parallel subuniverse
	  ****************************/

    void start();
  
    char machineFileNameBase[_POSIX_PATH_MAX];
    char machineFileName[_POSIX_PATH_MAX];

      /** create and delete machine file for Parallel from resource ads */
    void createMachineFile();
    void deleteMachineFile();
