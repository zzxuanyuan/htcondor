								...
								const char *addr, const char* claim_id )
{
	...
	if( param_boolean("SEC_SESSION_FROM_MATCH",false) ) {
		if( m_claim_session.secSessionId() == NULL ) {
			dprintf(D_ALWAYS,"SEC_SESSION_FROM_MATCH: failed to create security session from claim id, because claim id does not contain session information: %s\n",m_claim_session.publicClaimId());
		}
		else {
			...
			if( !rc ) {
				dprintf(D_ALWAYS,"SEC_SESSION_FROM_MATCH: failed to create security session for %s, so will fall back on security negotiation\n",m_claim_session.publicClaimId());
			}
			...
			if( !rc ) {
				...
				if( !rc ) {
					dprintf(D_ALWAYS, "SEC_SESSION_FROM_MATCH: failed to get session info for claim id %s\n",m_filetrans_session.publicClaimId());
				}
				else {
...
{
	if( !param_boolean("SEC_SESSION_FROM_MATCH",false) ) {
		dprintf(D_ALWAYS,"Request for security session info from starter, but SEC_SESSION_FROM_MATCH is not True, so ignoring.\n");
		return false;
	}
