						...
						const int peerIP, const short peerPort)
{
	...
	unsigned char buffer[BND_SZ_CONN];
	int index = 1;
	char pad = 0x00;
...
{
	unsigned char buffer[BND_SZ_BAND];
...
ReliSock::greetMnger()
{
    ...
    sockAddr.sin_port = htons((u_short)mngerPort);
...
int 
ReliSock::get_bytes_nobuffer(unsigned char *buffer, int max_length, int receive_size)
{
    int result;
    int length;
	unsigned char * buf = NULL;
...
{
    unsigned char buffer[BND_SZ_ALLOC-1];
    long long_param = 0;
    short short_param = 0;
    long sec, bytes;
    float percent = 0.0;
