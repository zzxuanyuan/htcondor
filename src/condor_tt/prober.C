...
Prober::probe()
{
	...
	if (fstat.st_mtime >= jqfile_last_mod_time) {
			...
			// here we need a way to tell whether the job queue file has been completed 
			// rewritten. If it's completed rewritten, we need to truncate the tables 
			// and re-load all data from the current job queue file. Otherwise, we only
			// need to check if the job queue file has increased, if so,  it means new 
			// operations need to be applied to the database.
