...
Scheduler::jobExitCode( PROC_ID job_id, int exit_code ) 
{
	...
	if ( ! is_goodput && ! is_badput) {
		...
		int job_start_xfer_out_date = -1;
		if ((!GetAttributeInt(job_id.cluster, job_id.proc, ATTR_USER_STAGEOUT_START, &job_start_xfer_out_date)
			&& job_start_xfer_out_date >= job_start_date) ||
			(!GetAttributeInt(job_id.cluster, job_id.proc, ATTR_JOB_CURRENT_START_TRANSFER_OUTPUT_DATE, &job_start_xfer_out_date)
			&& job_start_xfer_out_date >= job_start_date))
		{
