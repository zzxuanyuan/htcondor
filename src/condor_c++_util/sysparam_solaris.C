...
struct nlist solarisSpecific::nl[] =
    {
        { "_avenrun" },
        { "_anoninfo" },
        { "_physmem" },
...
int solaris::readSysParam(const SysParamName sys, char*& buffer, int& size,SysType& t)
{
	...
	case LoadAvg:						// lookup_load_avg()
        ...
        if (kvm_read (specific.kd, specific.nl[X_AVENRUN].n_value, 
			/*Solaris specific change ..dhaval 6/28 */			(char *)avenrun, sizeof (avenrun)) != sizeof(avenrun))
        {
...
/*solaris specific change 6/28 */(char *)&a_info, sizeof (a_info)) != sizeof(a_info))
		{
                kvm_close( specific.kd );
                specific.kd = NULL;
                set_condor_euid();
	...
	case PhyMem:						//  calc_phys_memory()
		unsigned int	phy;
		if (kvm_read (specific.kd, solarisSpecific::nl[X_PHYMEM].n_value, 
				/*Solaris specific change ..dhaval 6/28 */ (char *)&phy, sizeof (phy)) != sizeof(phy))
        {
                kvm_close( specific.kd );
                specific.kd = NULL;
                set_condor_euid();
...
void
solaris::disSysParam(const SysParamName sys, const char* buffer, 
