...
extern JobCollectionType h_jobs;
...
process ( const HistoryEntry &entry )
{
    ...
    tmp.sprintf ( "%d.%d", entry.cluster, entry.proc );

    dprintf ( D_FULLDEBUG, "HistoryProcessingUtils process: '%s'\n", tmp.Value() );

    const char *tmp2 = strdup(tmp.Value());

    HistoricalJob *job = new HistoricalJob ( tmp2, entry );
    h_jobs[tmp2] = job;

    JobCollectionType::const_iterator element = g_jobs.find ( tmp2 );
    if ( g_jobs.end() != element )
    {
	dprintf ( D_FULLDEBUG, "Job '%s' still active in job queue\n", tmp2 );
        return;
    }
    else {
      dprintf ( D_FULLDEBUG, "Job '%s' added to g_jobs\n", tmp2);
      g_jobs[tmp2] = job;
      h_jobs.erase(tmp2);
    }
