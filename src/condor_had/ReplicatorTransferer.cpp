...
bool
ReplicatorTransferer::killIfStuck( int sig, time_t now ) const
{
	if (  (m_pid > 0)  &&  (now >= m_time_limit) ) {
        return daemonCore->Send_Signal( m_pid, sig );
	};
	return true;
}
...
bool
ReplicatorTransfererList::killAllStuck(
	int			sig,
	time_t		now ) const
{
	bool	ok = true;
...
bool
ReplicatorTransfererList::killAll( int sig ) const
{
	bool	ok = true;
	list <ReplicatorTransferer *>::const_iterator iter;
	for( iter = m_list.begin(); iter != m_list.end(); iter++ ) {
