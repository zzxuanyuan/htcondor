...
my $warningLogPath               = $warningLogsDirectory . "/WarningLog-$currentTime";
...
my $consolidatedWarningLogPath   = $warningLogsDirectory . "/ConsolidatedWarningLog";
...
my $warningHistoryLogPath        = $warningLogsDirectory . "/WarningHistoryLog";
...
my @condorParameters = `condor_config_val HAD_LIST HAD_USE_PRIMARY HAD_CONNECTION_TIMEOUT`;
...
&InitializeStateFile($#logFilePaths);
...
&GenerateEpocheFile(\@eventFilesHandles, \@statusVector, \@timestampsVector,
		    \@eventsVector, $previousTimestamp, $currentTimestamp);
...
{
	my $totalErrorsNumber   = &ReturnFileLinesNumber($consolidatedErrorLogPath);
	my $totalWarningsNumber = &ReturnFileLinesNumber($consolidatedWarningLogPath);
	...
	&AttachFileAsText($message, $consolidatedEpocheLogPath, 'Consolidated epoche log') 
		if(-f $consolidatedEpocheLogPath);
	&AttachFileAsText($message, $consolidatedWarningLogPath, 'Consolidated warning log')
		if(-f $consolidatedWarningLogPath);
	&AttachFileAsText($message, $consolidatedErrorLogPath, 'Consolidated error log')
...
#        my @errorLogMessage = &ReturnFileContent($errorLogPath);
	...
	my $message = MIME::Lite->new(From    => FICTIVE_SENDER_ADDRESS, 
				      ...
				      Subject => "HAD Monitoring System Report - $errorsNumberAsString/$warningsNumberAsString",
				      Type    => 'multipart/mixed');
	$message->attach(Type => 'TEXT',
  			 Data => $messageBody);
        $message->attach(Type => 'TEXT',
...
#	$message->attach(Type => 'TEXT',
#			 Data => "Error log:\n\n@errorLogMessage");
	&AttachFileAsText($message, $errorLogPath  , 'Error log')   if -f $errorLogPath;
	&AttachFileAsText($message, $warningLogPath, 'Warning log') if -f $warningLogPath;
...
sub CheckValidity
{
	&AppendTextToActivityLog("Started checking validity of the epoche file\n");
...
#	my @hadSinfulStrings = split(',', $hadList);
	...
	foreach my $epocheLogLine (@epocheLogContent)
	{
		...
		foreach my $machineIndex (0 .. $machinesNumber)
		{
			...
			{
				$message .= $comma . $hadSinfulStrings[$machineIndex] . ' went down';
				$comma    = ',';
			}
		}
...
#	my $newLineChar = "";
	...
	while(<$logFileHandle>)
	{
		...
		{
			my $message = 'In log of ' . $hadSinfulStrings[$index] . ' there is a gap between ' . 
