...
StartdBenchJobParams::Initialize( void )
{
	...
	if ( NULL == m_libexec ) {
		const char	*libexec = "libexec";
		char		*release = param( "RELEASE_DIR" );
		if ( NULL == release ) {
			dprintf( D_ALWAYS,
					 "Job '%s': Neither LIBEXEC nor RELEASE_DIR defined\n",
					 GetName() );
			return false;	// Bail out
		}
		unsigned  len = strlen(release) + 1 + strlen(libexec) + 1;
		char	 *dir = (char *)malloc( len );
		if ( NULL == dir ) {
			dprintf( D_ALWAYS,
					 "Job '%s': Failed to allocate libexec (%d bytes)\n",
					 GetName(), len );
			return false;
		}
		strcpy( dir, release );
