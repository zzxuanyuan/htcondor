...
*/
int 
HistorySnapshot::sendQuery(SQLQuery *queryhor, SQLQuery *queryver, bool longformat)
{
  int st;
  ...
  st = jqDB->queryHistoryDB(queryhor, queryver, longformat, historyads_hor_num, historyads_ver_num); 
...
int
HistorySnapshot::printResults(SQLQuery *queryhor, SQLQuery *queryver, bool longformat) {
  AttrList *ad = 0;
...
HistorySnapshot::getNextAd_Hor(AttrList*& ad)
{
	...
	char *expr;
//printf("1 cur_historyads_index = %d\n", cur_historyads_index);
...
//printf("2 cur_historyads_index = %d\n", cur_historyads_index);
...
//printf("3 cur_historyads_index = %d\n", cur_historyads_index);
		cid = jqDB->getHistoryVerValue(cur_historyads_ver_index, 0); // cid
		pid = jqDB->getHistoryVerValue(cur_historyads_ver_index, 1); // pid
//printf("4 cur_historyads_index = %d\n", cur_historyads_index);
