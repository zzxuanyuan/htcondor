...
main_init(int argc, char *argv[])
{
	...
	stork = new Stork(mySubSystem);
	ASSERT(stork);
	stork->initialize();

	return 0;	// note: DC ignores the return value
}

//-------------------------------------------------------------

int 
main_config( bool is_full_reconfig )
{
	dprintf(D_ALWAYS, "main_config(%d) called\n", is_full_reconfig);
	stork->configure( is_full_reconfig );
	return 0;	// note: DC ignores the return value
}
...
main_shutdown_fast( void )
{
	...
	stork->shutdown(false);
	delete stork;
	DC_Exit(0);
	return 0;	// to satisfy c++.  note: DC ignores the return value
}
