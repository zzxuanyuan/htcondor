		...
		while( (schedd = scheddAds.Next()) )
		{
			...
			if ( num_idle_jobs == 0 ) {
				...
				if ( scheddLimit < 1 && spin_pie > 1 ) {
					...
					int numMatched = 0;
					startTime = time(NULL);
					result=negotiate( scheddName.Value(),scheddAddr.Value(),scheddPrio,
...
int Matchmaker::
negotiate( char const *scheddName, char const *scheddAddr, double priority, double share,
		   int scheddLimit,
...
ClassAd *Matchmaker::
matchmakingAlgorithm(char const *scheddName, char const *scheddAddr, ClassAd &request,
					 ClassAdList &startdAds,
...
matchmakingProtocol (ClassAd &request, ClassAd *offer, 
					    ...
					    char const* scheddName, char const* scheddAddr)
{
	int  cluster, proc;
	MyString startdAddr;
	char remoteUser[512];
