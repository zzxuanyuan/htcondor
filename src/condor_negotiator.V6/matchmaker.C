			...
			ClassAdList &startdAds, ClassAdList &startdPvtAds)
{
	...
	for (i = 0; i < scheddLimit;  i++)
	{
		...
		while (result == MM_BAD_MATCH) 
		{
			...
			if (job_universe == STANDARD || job_universe == RTLINK) {
				float cpu_time;
				...
				if (cpu_time > 0.0) {
					// if job_universe is STANDARD or RTLINK
					// (checkpointing is enabled) and cpu_time > 0.0
					// (job has committed some work), then the job
					// will	need to read a checkpoint to restart, so
					// we set the placement cost to be the image size,
					// which includes the executable size.
			...
			if (job_universe == STANDARD || job_universe == RTLINK ) {
				// if preempted job is a STANDARD or RTLINK universe
				// job, it will need to write a checkpoint, so we
				// include image size in the preemption cost
