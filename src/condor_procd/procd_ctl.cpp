...
dump(ProcFamilyClient& pfc, int argc, char* argv[])
{
	...
	for (int i = 0; i < count; i++) {
		printf("%u %u " PROCAPI_BIRTHDAY_FORMAT " %ld %ld %u %u %u\n",
		       (unsigned)elements[i].pid,
		       (unsigned)elements[i].ppid,
		       elements[i].birthday,
		       elements[i].user_time,
		       elements[i].sys_time,
		       (unsigned)elements[i].family_pid,
		       (unsigned)elements[i].family_watcher_pid,
		       (unsigned)elements[i].parent_family_pid);
	}
	return 0;
}
static int
