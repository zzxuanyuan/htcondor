...
class ProcDInterface:
    ...
    def dump(self):
        p = subprocess.Popen((self.procd_ctl_path, 'DUMP'),
...
class ProcDTester:
    ...
    def __init__(self):
        ...
        if platform.system() == 'Windows':
            ...
            pipe_name = '$(LOG)/procd_pipe'

        os.environ['PROCD_CONFIG'] = '/dev/null'
        os.environ['_PROCD_LOG'] = os.getcwd()
        os.environ['_PROCD_PROCD_ADDRESS'] = pipe_name
        os.environ['_PROCD_PROCD_MAX_SNAPSHOT_INTERVAL'] = '-1'
        os.environ['_PROCD_PROCD_LOG'] = '$(LOG)/procd_log'

        self.drone_tree = DroneTree()

        init_pid = self.drone_tree.get_init_pid()
        procd_pid = self.drone_tree.spawn(init_pid, './procd')

        self.procd_reference = ProcDReference(init_pid)
        self.procd_reference.spawn(procd_pid, init_pid)

        self.procd_interface = ProcDInterface()
...
if __name__ == '__main__':
	...
	# give time for the ProcD to get up and running
    time.sleep(1)
