		...
		if (result->argc == 2) {
			...
			if (rc == 1) {
				if (result->argc == 3) {	
					...
					// more than 3 parameter when return value = 1
					EXCEPT( "Bad %s result", command );
				}
			}
			else {
				// get groups' names
				for (int i=2; i<result->argc; i++) {
					group_names.append(result->argv[i]);
				}
				group_names.rewind();
			}
		}
	...
	if ( result ) {
		...
		if (result->argc == 2) {
			...
			if (rc == 1) {
				...
				// get keypairs' names
				for (int i=2; i<result->argc; i++) {
					keypair_names.append(result->argv[i]);
				}
	...
	if ( result ) {
		...
		if (result->argc == 2) {
			...
			if (rc == 1) {
				...
				// get keypairs' names
				for (int i=2; i<result->argc; i++) {
					bucket_names.append(result->argv[i]);
				}
				bucket_names.rewind();
			}
		}
	...
	if ( result ) {
		...
		if (result->argc == 2) {
			...
			if (rc == 1) {
				...
				// get keypairs' names
				for (int i=2; i<result->argc; i++) {
					entry_names.append(result->argv[i]);
				}
				entry_names.rewind();
			}
		}
								...
								const char * keyname, const char * outputname)
{
	...
	// outputname is optional. if client doesn't assign keyname, we should assign keyname to it
	if (outputname == NULL) 
		outputname = strdup(keyname);
