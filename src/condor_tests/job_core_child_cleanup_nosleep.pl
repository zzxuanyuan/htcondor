...
open(POUT, "> job_core_child_cleanup_nosleep.data") or die "Could not open file 'job_core_child_cleanup_nosleep.data': $?";
print POUT "Parent's pid is $$\n\n";

system("rm -rf job_core_child_cleanup_nosleep.data.kids");
...
while ($count < 10) 
{
	...
	if ($pid == 0)
	{
		...
		open(GCPOUT, ">> job_core_child_cleanup_nosleep.data.kids") or die "Could not open file 'job_core_child_cleanup_nosleep.data.kids': $?";
