	...
	while(length($remaining_text)>1) {
		...
		if ( $line =~ /^\s*NAMETABLE_DIRECTIVE:/ ) {
			if ( $line =~ /:BEGIN_SECTION:([a-zA-Z_]*)/ ) {
				...
				$options{class} = $1;
			} elsif ( $line =~ /:DECLARE_CLASS:([a-zA-Z_0-9]+)/ ) {
				$options{declare_class} = $1;
...
sub reconstitute {
	...
	my $declare_class = 1;
	$declare_class = $options{declare_class} if (exists $options{declare_class});
	...
	if ($declare_class) {
		continue_output( "typedef struct ${class_name} {\n"
						."   int id;\n"
						."   const char * name;\n"
						."   bool operator<(const struct ${class_name}& rhs) const { return id < rhs.id; }\n"
