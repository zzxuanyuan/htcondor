...
check_regex_match( char * pat, char * str )
{
	...
	if( rc == false ) {
        dprintf(D_SECURITY, "Error compiling pcre in check_regex_match." );
        dprintf(D_SECURITY, "Error in '%s' at %d: '%s'.", pat, err_offset, err );
		return 0; // TODO: is this the right thing to do?
    }
	rc = re.match(str);
    if( rc == FALSE ) {
        dprintf(D_SECURITY, "Fail: %s %s\n", pat, str );
        return 0;
    } else {
        dprintf(D_SECURITY, "Match: %s %s\n", pat, str );
        return 1;
    }
}
...
list_signing_policy( Policy * pol )
{
    ...
    while ( 1 ) {
        ...
        dprintf(D_SECURITY, "Policy element: %s\n", cur->policy_line );
        cur = cur->next;
    }
}
...
check_signing_policy( Policy * pol, char * user, char * subject )
{
    ...
    if( index( user, ':' ) != NULL ) {
        dprintf(D_SECURITY, "User authentication information contains illegal ':'.\n" );
        dprintf(D_SECURITY, "The user is: '%s'\n", user );
        return 0;
    }
    if( index( subject, ':' ) != NULL ) {
        dprintf(D_SECURITY, "Certificate request subject contains illegal ':'\n" );
        dprintf(D_SECURITY, "The subject is: '%s'\n", subject );
    }
    ...
    while( cur != NULL ) {
        if(check_regex_match( cur->policy_line, buf ) ) {
            // dprintf(D_SECURITY, "Got match: %s %s\n", user, subject );
            return 1;
        }
        cur = cur->next;
    }
...
read_signing_policy( )
{
    ...
    if( (fp = safe_fopen_wrapper( _mca_signing_policy_file, "r" )) ) {
        ...
        while( 1 ) {
            ...
            if( line_ptr == 0 ) {
                dprintf(D_SECURITY, "Error in malloc.\n" );
                exit( 1 );
            }
            ...
            if( strlen( line_ptr ) > 0 ) {
                ...
                if( policy_cur == NULL ) {
                    dprintf(D_SECURITY, "Error mallocing policy.\n" );
                    exit( 1 );
                }
...
get_subject_from_x509_req( char * csr )
{
    ...
    if(csr_text == NULL) {
        dprintf(D_SECURITY, "Error in malloc.\n" );
        dprintf(D_SECURITY, "In get_subject_from_x509_req" );
        free( csr_text );
        return NULL;
    }

    if( _mca_path_to_openssl == NULL ) {
        dprintf(D_SECURITY, "Error: _mca_path_to_openssl not defined.\n" );
        return NULL; 
    }

    if( pipe( pi_resp ) || pipe( pi_req ) ) {
        dprintf(D_SECURITY, "Error creating pipe.\n" );
        dprintf(D_SECURITY, "In get_subject_from_x509_req" );
        free( csr_text );
        return NULL;
    }
    ...
    pid = fork( );
    if( pid == -1 ) {
        dprintf(D_SECURITY, "Error forking in get_subject_from_x509_req.\n" );
        free( csr_text );
        return NULL;
    }
    if( pid == 0 ) { // child
        ...
        dprintf(D_SECURITY, "Error execing!\n" );
        free( csr_text );
        return NULL;
    }
    ...
    if( size < csr_size ) {
        dprintf(D_SECURITY, "Couldn't write to child.\n" );
        dprintf(D_SECURITY, "In get_subject_from_x509_req" );
        free( csr_text );
        return NULL;
    }
    ...
    if( csr_size < 1 ) {
        dprintf(D_SECURITY, "Didn't read anything.\n" );
        dprintf(D_SECURITY, "In get_subject_from_x509_req" );
        free( csr_text );
        return NULL;
    }
    ...
    if( subject == NULL ) {
        dprintf(D_SECURITY, "Error: didn't find 'Subject: ' in csr: \n%s\n",
                 csr );
        free( csr_text );
        return NULL;
    }
    ...
    if( subject_rv == NULL ) {
        dprintf(D_SECURITY, "Couldn't malloc for subject return value.\n" );
        dprintf(D_SECURITY, "In get_subject_from_x509_req" );
        free( csr_text );
        return NULL;
    }       
    ...
    if( status < 1 ) {
        dprintf(D_SECURITY, "In get_subject_from_x509_req" );
        dprintf(D_SECURITY, "Couldn't copy string\n" );
        free( csr_text );
        free( subject_rv );
        return NULL;
    }
    ...
    do {
        if( waitpid(pid, &status, 0) == -1 ) {
            if( errno != EINTR ) {
                dprintf(D_SECURITY, "In get_subject_from_x509_req" );
                dprintf(D_SECURITY, "Problem, can't wait right.\n" );
                free( subject_rv );
                return NULL;;
            }
        } else {
...
sign_req( char *csr, int *err_code )
{
    ...
    *err_code = MCA_ERROR_NOERROR;
    ...
    if( cert == NULL || err_text == NULL ) {
        dprintf(D_SECURITY, "Error in malloc.\n" );
        dprintf(D_SECURITY, "In sign_req" );
    ...
    if( size < csr_size ) {
        dprintf(D_SECURITY, "In sign_req" );
        dprintf(D_SECURITY, "Error writing csr file.\n" );
    ...
    if(pid == -1) {
        dprintf(D_SECURITY, "In sign_req" );
        dprintf(D_SECURITY, "Error forking.\n" );
    ...
    // don't try too hard
	// dprintf(D_SECURITY,"Here's where we're writing the password: '%s'\n", _mca_ca_password);
    offset = write( ca_resp[1], _mca_ca_password, strlen( _mca_ca_password ) );
    if( offset != (int) strlen( _mca_ca_password ) ) {
        dprintf(D_SECURITY, "Error writing ca password.\n" );
        exit( 1 );
    }
	offset = write( ca_resp[1], "\n", 1 );
	if( offset != 1 ) {
		dprintf(D_SECURITY, "Error writing ca password newline.\n" );
		exit( 1 );
	}
    ...
    // dprintf(D_SECURITY, "Read %d.\n", cert_size );

    if( offset < 0 ) {
        dprintf(D_SECURITY, "Error reading.\n" );
	...
	//dprintf(D_SECURITY,"All done, just waiting.\n");
    if( waitpid(pid, &status, 0) == -1 ) {
        dprintf(D_SECURITY, "In sign_req" );
        dprintf(D_SECURITY, "Problem, can't wait right.\n" );
    ...
    if( WIFEXITED( status ) ) {
        dprintf(D_SECURITY, "openssl exited with code %d.\n",
                 WEXITSTATUS( status ) );
        if( WEXITSTATUS( status ) != 0 ) {
            dprintf(D_SECURITY, "Error:\n%s", err_text );
            if( NULL != strstr( err_text, "TXT_DB error number 2" )) {
                dprintf(D_SECURITY, "Certificate request conflicts with existing cert (%s).\n", cert );
    ...
    if( unlink( csr_file ) ) {
        dprintf(D_SECURITY, "In sign_req" );
        dprintf(D_SECURITY, "Couldn't unlink file!\n" );
    }
    return( cert );
...
check_version_compatibility( Stream *sock )
{
    ...
    if( ! (sock->code( minica_client_version )) ) {
        dprintf(D_SECURITY, "check_version_compatability() received bad input.\n" );
        return FALSE;
    }
    ...
    if( ! (sock->code( minica_server_version )) ) {
        dprintf(D_SECURITY, "check_version_compatability() error sending version.\n" );
    ...
    if( ! strcmp( minica_server_version, minica_client_version ) ) {
        ...
        if( ! ( (sock->code( response_code ))
                ...
                &&( sock->end_of_message( )) ) ) {
            dprintf(D_SECURITY, "check_version_compatability() error in protocol init.\n" );
            dprintf(D_SECURITY, "Client: '%s'\nServer: '%s'\n",
        ...
        sock->code( error_message );
        sock->end_of_message( );
        dprintf(D_SECURITY, "check_version_compatability() aborting due to version mismatch.\n" );
        free( error_message );
        free( minica_client_version );
        free( minica_server_version );
        return FALSE;
...
handle_passive_client( Stream *sock )
{
    ...
    //dprintf(D_SECURITY, "Entering handle_passive_client.\n" );

    sock->decode( ); /* 3 */
    if( ! ( (sock->code( passive_client ))
            && (sock->end_of_message( )) ) ) {
        dprintf(D_SECURITY, "handle_passive_client() received bad input.\n" );
        return FALSE;
    }
    ...
    if( ! passive_client ) {
        ...
        dprintf(D_SECURITY, "Telling active client to proceed.\n" );
        if( ! ( (sock->code( response_code ))
                && (sock->code( error_message ))
                && (sock->end_of_message( )) ) ) {
            dprintf(D_SECURITY, "handle_passive_client() error sending passive accept.\n" );
            free( error_message );
            return FALSE;
        }
        free( error_message );
        ...
        if( (! MCA_ACCEPT_PASSIVE_CLIENTS) && passive_client ) {
            ...
            sock->end_of_message( );
            free( error_message );
            dprintf(D_SECURITY, "handle_passive_client() won't accept passive clients.\n" );
            return FALSE;
        }
        ...
        if( ! ( (sock->code( pass ))
                ...
                && (sock->end_of_message( )) ) ) {
            dprintf(D_SECURITY, "handle_passive_client() received bad input (5).\n" );
            if( pass != NULL ) {
                free( pass );
                pass = NULL;
            }
        ...
        dprintf(D_SECURITY, "Generating client key.\n" );
        ...
        if( error_code > MCA_ERROR_NOERROR ) {
            dprintf(D_SECURITY, "Error generating key: %s\n",
                     get_mca_error_message( error_code ) );
        }
        if( key_block == NULL ) {
            response_code = MCA_RESPONSE_KEY_FAILED;
        } else {
            dprintf(D_SECURITY, "Generating client crt.\n" );
        ...
        case MCA_RESPONSE_CSR_FAILED:
            ...
            break;
        default:
            dprintf(D_SECURITY, "Sanity check.  Unknown response code." );
            error_message = NULL;
        }
        if( error_message == NULL ) {
            free( key_block );
        ...
        //dprintf(D_SECURITY, "Sending key and csr to client:\n%s\n%s\n%s.\n", error_message, key_block, csr );
        sock->encode( ); /* 6 */
        if( ! ( (sock->code( response_code ))
                && (sock->code( error_message ))
                && (sock->code( key_block ))
                && (sock->code( csr ))
                && (sock->end_of_message( )) ) ) {
            dprintf(D_SECURITY, "handle_passive_client() error sending.\n" );
        }
...
sign_cert( Stream *sock, char *user )
{
    ...
    sock->decode(); /* 7 */
    dprintf(D_SECURITY, "Receiving csr from client.\n" );
    if( ! ( (sock->code( csr )) &&
            (sock->end_of_message( )) ) ) {
        dprintf(D_SECURITY, "sign_cert() received bad input (7).\n" );
        return FALSE;
    }
    if( csr == NULL ) {
        dprintf(D_SECURITY,
    ...
    if( subject == NULL ) {
        dprintf(D_SECURITY, "sign_cert() can't get subject for cert '%s'\n", csr );
        response_code = MCA_RESPONSE_CERT_FAILED;
    } else {
        dprintf(D_SECURITY, "Received signing request with subject: '%s'\n", subject );
        
        if( ! check_signing_policy( policy, user, subject ) ) {
            dprintf(D_SECURITY, "Policy check failed.\n" );
            dprintf(D_SECURITY,
            ...
            if( error_code > MCA_ERROR_NOERROR ) {
                dprintf(D_SECURITY, "Error signing certificate request: %s\n",
                         get_mca_error_message( error_code ) );
            }
    ...
    if( ! ( (sock->code( response_code )) &&
            ...
            (sock->code( cert )) &&
            (sock->end_of_message( )))) {
        dprintf(D_SECURITY, "sign_cert() error sending.\n" );
    }
...
get_lock( int *lock_fd ) {
    ...
    *lock_fd = safe_open_wrapper( _mca_ca_op_lock_file, 
                        ...
                        S_IRUSR|S_IWUSR );
    if( *lock_fd == -1 ) {
        dprintf(D_SECURITY, "In get_lock" );
        dprintf(D_SECURITY, "Error creating lock file.\n" );
        return FALSE;
    }
    dprintf(D_SECURITY, "Opened lock file...\n" );
    
    if( flock( *lock_fd, LOCK_EX ) == -1 ) {
        dprintf(D_SECURITY, "In get_lock" );
        dprintf(D_SECURITY, "Error creating lock on lock file.\n" );
        if( close( *lock_fd ) == -1 ) {
            dprintf(D_SECURITY, "In gen signing request" );
            dprintf(D_SECURITY, "Fatal: error closing lock file.\n" );
            return FALSE;
        }
        return FALSE;
    }
    dprintf(D_SECURITY, "Got lock on lock file %d.\n", *lock_fd );
    return TRUE;
}
...
{
    //dprintf(D_SECURITY, "Releasing lock.\n" );
    if( flock( lock_fd, LOCK_UN ) == -1 ) {
        // We can't sign anything else, so we exit.
        dprintf(D_SECURITY, "In release_lock" );
        dprintf(D_SECURITY, "Error removing lock from lock file.\n" );
        return FALSE;
    }
    if( close( lock_fd ) == -1 ) {
        dprintf(D_SECURITY, "In release_lock" );
        dprintf(D_SECURITY, "Error closing lock file.\n" );
        return FALSE;
    }
    //dprintf(D_SECURITY, "Released lock.\n" );
    return TRUE;
}
...
send_openssl_cnf( Stream *sock )
{
    ...
    dprintf(D_SECURITY," Entering send_openssl_cnf.\n" );
    
    openssl_cnf = file_to_str( _mca_path_to_openssl_cnf, &error );
    if( openssl_cnf == NULL ) {
        dprintf(D_SECURITY, "send_openssl_cnf error: '%s'\n", get_mca_error_message( error ) );
        return FALSE;
    }
    sock->encode( );
    if( ! (sock->code( openssl_cnf )) ) {
        dprintf(D_SECURITY, "send_openssl_cnf() error sending file.\n" );
        free( openssl_cnf );
        return FALSE;
    }
    if( ! (sock->end_of_message( )) ) {
        dprintf(D_SECURITY, "send_openssl_cnf() error communicating.\n" );
        free( openssl_cnf );
        return FALSE;
    }
    free( openssl_cnf );
    dprintf(D_SECURITY,"Sent openssl.cnf to client successfully.\n" );
    return TRUE;
}
...
{
    int xfer = FALSE;
    dprintf(D_SECURITY, "Transfering config file status.\n" );
    sock->decode( );
    if( ! ( (sock->code( xfer ))
            && (sock->end_of_message( )) ) ) {
        dprintf(D_SECURITY, "Error getting config file status.\n" );
        return FALSE;
    }
    if( xfer ) {
...
//        }
	...
	if(!(socket->getFullyQualifiedUser())) {
        dprintf(D_SECURITY, "sign_cert_request() Error: couldn't get user.\n" );
        return FALSE;
    }
    ...
    dprintf(D_SECURITY, "sign_cert_request: Connection initiated by: '%s'\n", user );
    
    /* Let the daemon_core get back to its event loop */
    pid = fork();
    if( pid == -1 ) { // can't fork.  
        dprintf(D_SECURITY, "In sign_cert_request" );
        dprintf(D_SECURITY, "Error forking.\n" );
        free( user );
        return FALSE;
    }
    if( pid != 0) {
        dprintf(D_SECURITY, "parent returning to main event loop.\n" );
        return TRUE;
    }
    ...
    if( check_version_compatibility( sock )
        ...
        free( user );
        dprintf(D_SECURITY, "completed with success, exiting child.\n" );
        exit( 0 );
    }
    release_lock( lock_fd );
    free( user );
    dprintf(D_SECURITY, "completed with failure, exiting child.\n" );
    exit( 1 ); //DC_Exit( 1 );
}
...
usage( char * MyName )
{
    dprintf(D_SECURITY, "Usage: %s [option]\n", MyName );
    dprintf(D_SECURITY, "  where [option] is one of:\n" );
    dprintf(D_SECURITY, "    -m filename  (sinful address of daemon is written to filename)\n" );
...
param_contains( char *name, char *needle )
{
    ...
    if( strstr( tmp, needle ) ) {
        ...
        dprintf(D_SECURITY, "'%s' does not contain '%s'; it is '%s'.\n",
                     name, needle, tmp );
    }
    free( tmp );
    return rv;
...
param_is( char *name, char *needle )
{
    ...
    if( ! strncmp( needle, tmp, strlen( needle ) ) ) {
        ...
        dprintf(D_SECURITY, "'%s' is not '%s'; it is '%s'.\n",
                     name, needle, tmp );
    }
    free( tmp );
    return rv;
...
main_config( bool is_full )
{
	...
	dprintf(D_SECURITY, "main_config() called\n" );
    ...
    if( ! ( _mca_signing_policy_file &&
            ...
            _mca_ca_key_file &&
            _mca_ca_op_lock_file ) ) {
        dprintf(D_SECURITY, "configuration file must contain:\n" );
        if( ! _mca_signing_policy_file ) {
            dprintf(D_SECURITY, "MCA_SIGNING_POLICY_FILE - the location of the signing policy\n" );
        }
...
/* Shoot yourself!
    ...
    if( safe == 0 ) {
        dprintf(D_SECURITY, "Unsafe config settings.\n" );
       if( !param_is( "MCA_ALLOW_INSECURE_CA", "YES" ) ) {
           DC_Exit( 1 );
       } else {
           dprintf(D_SECURITY, "WARNING: Continuing.  MCA_ALLOW_INSECURE_CA=YES\n" );
       }
    }
*/
    ...
    if( policy == NULL ) {
        dprintf(D_SECURITY, "read_signing_policy returns null.  Exiting.\n" );
        DC_Exit( 1 );
        return FALSE;
    }
    dprintf(D_SECURITY, "Configuration completed successfully.\n" );
    dprintf(D_SECURITY, "Configuration summary:\n" );
    dprintf(D_SECURITY, "MCA_SIGNING_POLICY_FILE: %s\n", _mca_signing_policy_file );
    dprintf(D_SECURITY, "MCA_PATH_TO_OPENSSL: %s\n", _mca_path_to_openssl );
    dprintf(D_SECURITY, "MCA_PATH_TO_OPENSSL_CNF: %s\n", _mca_path_to_openssl_cnf );
    dprintf(D_SECURITY, "MCA_CA_OP_LOCK_FILE: %s\n", _mca_ca_op_lock_file );
    dprintf(D_SECURITY, "MCA_CA_KEY_FILE: %s\n", _mca_ca_key_file );
    if( _mca_ca_key_passin )
        dprintf(D_SECURITY, "MCA_CA_KEY_PASSIN: %s\n", _mca_ca_key_passin );
    else
        dprintf(D_SECURITY, "MCA_CA_KEY_PASSIN not defined: will prompt for password.\n" );

    dprintf(D_SECURITY, "Policy:\n" );
...
get_validated_ca_password( )
{
    ...
    if( passin ) {
        ...
        dprintf(D_SECURITY, "get_validated_ca_password: Couldn't get memory.\n" );
        exit( 1 );
    }
    if( _mca_ca_key_passin ) {
    ...
    if( key == NULL ) {
        dprintf(D_SECURITY, "Couldn't even begin to read key!\n" );
        exit( 1 );
    }
    ERR_print_errors_fp( stderr );
    if( BIO_read_filename( key, _mca_ca_key_file ) <= 0) {
        ERR_print_errors_fp( stderr );
        dprintf(D_SECURITY, "Couldn't read key from file '%s'.\n", _mca_ca_key_file );
        exit( 1 );
    }
    ERR_print_errors_fp( stderr );
    ...
    free( passin );
    if( pkey == NULL ) {
        dprintf(D_SECURITY, "Couldn't open private key file.\n" );
        ERR_print_errors_fp( stderr );
            return 0;
    } else {
...
main_init(int argc, char *argv[])
{
    ...
    for(ptr = argv + 1; *ptr; ptr++) {
        ...
        case 'm':
            ++ptr;
            if( !(ptr && *ptr) ) {
                dprintf(D_SECURITY, "-m requires another argument\n");
                usage( argv[0] );
            ...
            if(str_to_file( buf, *ptr ) == -1) {
                dprintf(D_SECURITY, "In main_init" );
                dprintf(D_SECURITY, "Error opening file for writing: ");
                usage( argv[0] );
            }
            break;
        default:
            dprintf(D_SECURITY, "Error: Unknown option %s\n", *ptr );
            usage( argv[0] );
        }
    }
    ...
    if( get_validated_ca_password( ) == 0 ) {
        dprintf(D_SECURITY, "Error: server password bad.\n" );
        exit( 1 );
    }
    ...
    daemonCore->Register_Command( DC_SIGN_CERT_REQUEST, "SIGN_CERT_REQUEST",
                                  ...
                                  D_FULLDEBUG );;
	dprintf(D_ALWAYS, "main_init completed.\n");
	return TRUE;
}
