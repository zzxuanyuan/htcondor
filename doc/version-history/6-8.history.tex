...
\item Performed a security audit of all places where Condor opens files to make certain files are opened with a reasonable permission mode and with the
...
\item The \Condor{schedd} now maintains a birthdate for the job queue. 
...
\item Fixed a deadlock situation between the schedd and startd than can
significantly impact the schedd's performance.  The likelyhood of the
...
occasional dagman confusion (thanks to Fairview Software, Inc, for
...
\item A new version of the GT4 GAHP is included, with the following changes:
  ...
  \item New axis.jar from Globus that fixes a thread safety bug that
  ...
  be reclaimed in both client and server.

  \item Removed redundant usage of Secure Message, Secure Conversation,
  and Transport Security when talking to a WS GRAM service. Now, just
...
\item Fixed bug that might have caused the \Condor{startd} problems
...
Previously, it would only transfer back files that had a timestamp newer
...
compilation when running in the Java universe with the following configuraiton
setting: \begin{verbatim} JAVA_EXTRA_ARGUMENTS = -Djava.compiler=""
\end{verbatim}
