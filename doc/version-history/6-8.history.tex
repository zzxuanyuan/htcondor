...
postgres database tables.  Some sites were reporting that even
...
Rebuilding is disabled by setting the parameter QUILL\_SHOULD\_REINDEX
to false.  Reindexing happens immediately after the history file
is purged of old data. So, if quill is configured to never delete
history data, the tables are never reindexed.  Also, \Condor{quill}
...
involved in file transfers: ``Download acknowledgment missing
attribute: Result.''  The extra information is a full dump of the
...
\item Garbage numbers (i.e. random bits from memory) were sometimes
being written to the pool history file in the field representing the
backfill state.
...
\item Fixed several bugs for grid-type nordugrid jobs.
...
on TCP updates to the collector if they are enabled on the submit machine.

\item When using the submit file option getenv=true, environment
variables containing a newline in their value are no longer inserted
into the job's environment.  \Condor{schedd} does not allow newlines
in ClassAd values, so the attempt to insert such values was resulting
in failure to submit the job (and was in fact causing \Condor{schedd}
to abort).
...
If the \Condor{schedd} is very busy, it is possible that when
a local universe job completes, the \Condor{starter} will not
be able to connect to the job queue to update final information
about the job (exit status, etc). Previously, when this happened,
the \Condor{starter} would hang indefinitely. Now, it will attempt
to retry a few times (with a delay in between each one) before
exiting with a fatal error (which would cause the job to restart).
