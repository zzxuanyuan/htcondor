...
\item This release fixes a security vulnerability that affects only
those who rely upon Condor's network message integrity checking
(e.g. \MacroNI{SEC\_DEFAULT\_INTEGRITY}=REQUIRED).  Not all of
...
the level of threat to be modest.  For users of the 6.9 development
series, a fix for this problem will be released as part of the new 7.0
stable series release, which is planned to happen near the end of
2007.
...
\item Fixed a named pipe collisions on Windows: streaming error and 
output would not work on more than one slot at a time.
...
that is forwarded from one collector to another via
...
(e.g. \MacroNI{ALLOW\_READ}) now properly recognize values where the
username contains a wildcard (e.g. \MacroNI{*@cs.wisc.edu/bird.cs.wisc.edu}).
...
other than then \Condor{credd}'s hostname, clients can still locate the
daemon.
