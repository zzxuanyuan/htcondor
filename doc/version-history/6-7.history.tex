...
% Implicity added PR 278, which requires Stork user logs to
...
specified in job submit file.  Stork now uses Condor user log output format,
including optional XML format.  Previous, per-server Stork user log in 
\File{LOG/Stork.user\_log} has been removed.  \Condor{dagman} has been updated
to read new per-job user logs.

% Gnats PR 600.
\item Added new per-job Stork input, output and error output file
specifications.  Stork job output is now optional, and
specified in job submit file.  Previous, per-server Stork user log in 
\File{LOG/Stork-module.stderr} and \File{LOG/Stork-module.stderr} has been
removed.
