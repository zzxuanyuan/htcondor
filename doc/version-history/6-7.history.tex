...
\item This release contains all the bug fixes from the 6.6 stable
  series up to and including version 6.6.8, and some of the fixes that
  will be included in version 6.6.9.
  The bug fixes in version 6.6.9 that were not included in version
  6.7.5 are listed in a separate section of the 6.6.9 version
  history. 
...
\item Condor now includes a SOAP interface to many of the Condor daemons
...
\item Jobs can now pass arguments to the underlying JVM with the \Attr{JavaVMArgs} attribute in a submit file
...
\item Fixed the Condor communications library to correctly re-assemble
fragmented messages that arrived out of order when encryption was 
active. 
Sites that are using secure channels should see fewer dropped updates. 

\item Fixed the \Condor{collector} so a machine may have multiple instances of the \Condor{schedd}. 
Previously, updates from each schedd would overwrite the submitter ad from
the other instance, and it would be a random choice which one the 
\Condor{negotiator} would matchmake with.

\item The \Opt{-pool} option to \Condor{config\_val} was broken in
  version 6.7.2 through version 6.7.4, and was effectively ignored. 
  Starting in version 6.7.5, this command-line option can once again
  be used to query and locate daemons in remote Condor pools.
...
\item The WSDL files for the SOAP interfaces are not included in this release.
This is an oversight by the Condor team in packaging, and they will be 
included in future releases.
You can get the WSDL files from the web separately.
...
\item This release of Condor never appeared on the Condor download site.
