...
Create a permanent account (called nobody-guest in this description)
under which Condor will run jobs.
On all Windows machines, and on the file server, create the
nobody-guest account.

On the network file server, give the nobody-guest user permissions
to access files needed to run Condor jobs.

Securely store the password of the nobody-guest user in the
Windows registry using \Condor{store\_cred}.

Tell Condor to use the nobody-guest user as the owner of jobs,
when required.
