...
\begin{enumerate}
  ...
  \item If the job had already run and checkpointed on machines
  before the upgrade, then using the \Attr{LastRemoteHost} attribute
  in the job's ClassAd determine the the \Attr{CheckpointPlatform}
  for the machine on which the job last ran. 
  Then use \Condor{qedit} to add the value of the machine's
  \Attr{CheckpointPlatform} attribute as the value of the new attribute
  \Attr{LastCheckpointPlatform} in the job's ClassAd.
  Note that this operation will also have to be performed for standard
  universe jobs flocking into a pool which have been upgraded. 
