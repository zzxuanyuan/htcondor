...
\subsection*{After an installation of Condor, why do the daemons refuse to 
	start giving this message in the logfiles?}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
As of Condor 6.8.0, a configuration file change has been made where if 
Condor sees the bare key-word: 
\Expr{YOU\_MUST\_CHANGE\_THIS\_INVALID\_CONDOR\_CONFIGURATION\_VALUE}
as a value of a config file entry, Condor daemons will produce the above
exception and exit.

By default, an installation of Condor 6.8.0 and later will have the
config file entry \Expr{HOSTALLOW\_WRITE} set to the above sentinel
value. 
The Condor administrator must alter this value to be the correct domain
or ip addresses that the administrator desires--the wildcard character
is allowed. You may use * as a value, but that allows anyone, from
anywhere, to submit jobs into your pool, among other things. A usual
entry could be something like *.domainname.com.
