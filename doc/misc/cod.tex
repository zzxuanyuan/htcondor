...
You can also provide a constraint on what virtual machine you wish to
use, instead of specifying it as the name.  
For example, if you wanted to run on machine c02.cs.wisc.edu, and you
didn't care which virtual machine you used so long as it wasn't
currently running a job, you could use something like this:
\begin{verbatim}
...
on the same resource will automatically become suspended.
...
claim will change (the new value will depend on why the application
became inactive, and will be described in detail below) and the batch
Condor job will resume.

The procedure to activate a COD claim is to define attributes about
the job to be run (in either the local configuration of the COD
resource, or in a separate file as described above in
section~\ref{sec:cod-setup}), and then invoke the
...
job attributes are defined in the Condor configuration file or are
passed via a file to the \Condor{cod\_activate} tool itself.
However, the \Opt{-id} option is always required for
\Condor{cod\_activate}, and should be followed by a COD claim ID that
the user acquired via \Condor{cod\_request}.

\Note In the examples in this section, whenever the example lists
\verb@"<claim_id>"@, the user must substitute a valid COD claim ID,
instead.

If the application is defined in the configuration files for the COD
resource, the user must provide the keyword (described in
...
\Condor{cod\_activate} tool (described in
section~\ref{sec:cod-command-line-attrs}) the user must provide the
name of the file using the \Opt{-jobad} option.
For example, if the job attributes were defined in a file named
\File{cod-fractgen.txt}, the user would spawn the job using the
following command:
\begin{verbatim}
...
Regardless of how the job attributes are defined, there are a few
other options that \Condor{cod\_activate} accepts.
These options specify the job ID for the application to be run.
The job ID can either be specified in the job's ClassAd (as described
above in section~\ref{sec:cod-application-attributes}) or it can be
specified on the command-line to \Condor{cod\_activate}.
These options are \Opt{-cluster} and \Opt{-proc}.
For example, to launch a COD job as 23.5, the user would invoke:
\begin{verbatim}
condor_cod_activate -id "<claim_id>" -key foo -cluster 23 -proc 5
\end{verbatim}
These arguments are optional, since the job ID is not required for
COD.
...
If there was a batch job on the resource when the COD claim was first
activated, the batch job will be suspended once the COD job starts
running.
By suspending the COD application, the batch job is allowed to run.
If the resource was idle when the COD application was first spawned,
once the COD job is suspended, the batch resource becomes available
for use in the Condor system and might be matched with an idle batch
job.
So, whenever a COD application has no work to perform, it should be
suspended to prevent the batch resource from being wasted.

The interface to \Condor{cod\_suspend} is simple.
The only option it supports is \Opt{-id} to specify the COD claim ID
the user wishes to suspend.
For example:
\begin{verbatim}
condor_cod_suspend -id "<claim_id>"
\end{verbatim}

If the user attempts to suspend a COD job that is not running (either
because it is already suspended, or because a job was never spawned on
the given COD claim in the first place) \Condor{cod\_suspend} will
exit with an error message.
...
The \Condor{cod\_resume} tool only supports the \Opt{-id} option to
...
\Condor{cod\_resume} will exit with an error message.
...
removed manually, the user can invoke the \Condor{cod\_deactivate}
command to kill the job, but leave the COD claim ID valid for future
COD jobs.
The user must specify the claim ID they with to deactivate using the
\Opt{-id} option.
For example:
\begin{verbatim}
...
tool will exit with an error message.
