...
controls which job user log
events are appended to the log referred to by \Attr{DAGManNodesLog} 
(the auxiliary log). 
If the mask is empty or undefined, these events are written to the
auxiliary job log:  
\begin{itemize}
...
\end{itemize}
Otherwise, this string is a comma-separated list of integer 
user log event codes;
only events found in the list are written to the auxiliary log. 
If \Attr{DAGManNodesLog} is
not defined, it has no effect. The value of \Attr{DAGManNodesMask} does not
affect events recorded in the user log file referred to by \Attr{UserLog}.
...
\Condor{submit} file via the accounting\_group command.  This attribute
...
\item[\AdAttr{RemotePool}:]  The name of the collector of the pool in which
a job is running via flocking.  This attribute is not defined if the job
is not running via flocking.

%%% ClassAd attribute: LastRemotePool
\index{ClassAd job attribute!LastRemotePool}
\index{LastRemotePool!job ClassAd attribute}
\item[\AdAttr{LastRemotePool}:]  The name of the collector of the pool in which
a job ran via flocking in the most recent run attempt.  This attribute is not
