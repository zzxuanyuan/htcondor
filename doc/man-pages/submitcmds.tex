...
%For example, to require that whatever machine executes a Condor job has 
...
Condor will write an SSH key for use with ec2 jobs. 
...
of grid-type gt2, gt2, cream, or nordugrid, then the value of
\SubmitCmd{use\_x509userproxy} is forced to \Expr{True}.
...
is recorded on the job ad as \AdAttr{AcctGroup}.
...
defaults to the value of \AdAttr{Owner}.  The group user value is
recorded on the job ad as \AdAttr{AcctGroupUser}.
...
match time for any given resource. This example allows Condor to automatically
