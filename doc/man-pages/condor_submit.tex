...
\item[MaxJobRetirementTime] Expression \index{MaxJobRetirementTime}
giving the maximum runtime of the job (in seconds) that should be
allowed for the job to finish naturally (with no soft or hard kill
signals) when the resource claim is being preempted.  If the job is
still running when the time expires, the usual preemption procedure is
followed (typically a soft kill signal, followed by some time to
gracefully shut down, followed by a hard kill signal).

Standard universe jobs and any jobs running with nice\_user priority
have a default \AdAttr{MaxJobRetirementTime} of 0.  In all other
cases, no default value is provided, so if \AdAttr{MaxJobRetirementTime}
is not specified, the full amount of time given by the machine policy
will apply.  If \AdAttr{MaxJobRetirementTime} in your submit file is
longer than the machine policy allows, the machine's upper bound takes
effect.
...
machines with some minimum \AdAttr{MaxJobRetirementTime} or if you
...
(assuming the signal identifier for segmentation fault is 4):

\begin{verbatim}
	on_exit_remove = (ExitBySignal == False) && (ExitSignal != 4)
\end{verbatim}

This expression will not let the job leave the queue,
if the job exited by a signal and
that signal number was 4 (representing segmentation fault).
...
submit event and a completetion event, with exit code 0, unless 
...
If a job has specificed it is to be a noop, this setting allows the job
...
Multiple environment variables can be specified by separating them with a
vertical bar (\Bar) when submitting from an NT platform.
...
\Opt{signal-number}.  \Opt{signal-number} needs to be an integer which
...
\Opt{signal-number} is the signal delivered
to the job when the job is removed
with \Condor{rm}.
\Opt{signal-number} may be either the platform-specific name or value
of the signal.
...
the value of \Opt{kill\_sig} is used. 
...
\Opt{x509userproxy} is only required when
the \Opt{universe} is \Opt{grid},
and \Opt{grid\_type} is one of \Opt{gt2}, \Opt{gt3}, or \Opt{nordugrid}.
...
\Opt{globusscheduler} is only required when
the \Opt{universe} is \Opt{grid},
and \Opt{grid\_type} is one of \Opt{gt2} or \Opt{gt3}.
...
The \Opt{host} may be specified by either a host name
(as in \File{head.example.com}) or an IP address
(of the form 123.456.7.8).
The \Opt{port} number is an integer.
...
As an alternative to using \Opt{MyProxyPassword} in the
submit description file,
...
For example, if \Opt{MyProxyRefreshThreshold} is set to the
