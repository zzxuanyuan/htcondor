...
Condor libraries.  \emph{Note:} Use the \Arg{vanilla} Universe to
...
This expression is checked every 20 seconds (currently non-configurable),
and if it becomes \Expr{True}, the job is removed from the queue.
...
is checked every 20 seconds(currently non-configurable),
and if it becomes true, the job will be placed on hold.
If unspecified, the default value is \Expr{False}.
...
is reevaluated every 20 seconds(currently non-configurable)
...
greater than zero (0),
attributes are placed in the job ClassAd.
...
If \Expr{False}, \File{stdout} is stored by the Globus jobmanager 
...
the machine from which the job was submitted. 
If \Expr{False}, \File{stderr} is stored by the Globus jobmanager 
and transferred back when the job completes.
This command is ignored if the job ClassAd attribute
\Attr{TransferErr} is
\Expr{False}.
...
file;
for example, by issuing an \Opt{initialdir} between each \Opt{queue}
