...
%% remote_pool
%%%%%%%%%%%%%%%%%%%
\item[remote\_pool = $<$machine-name$>$]
\index{submit commands!remote\_pool}
For a \Arg{grid} universe job, with a \Opt{grid\_type} of \Arg{condor},
the machine that receives a query from the submit machine.
The query is to the \Condor{collector} daemon
(assumed to be at a standard IP address and port),
and it is for the location (IP address and port) of the
