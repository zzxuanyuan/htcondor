...
\AdAttr{AccountingGroup} job ClassAd attribute, as described above,
except that \Arg{group}.\Arg{user} must be specified, not just \Arg{group}.

The submit file syntax for specifying a group user is
\begin{verbatim}
...
a required part of the syntax.  \Bold{Note:  if you specify an accounting
group value without the period, your job will not be considered part of the
group when negotiating, even if the group name you specify has a quota.
Furthermore, you will not receive any warnings that the group quota is not
in effect for your job.}  Group names are case-insensitive for
negotiation.
...
is true (this could happen, for example, if there are only 15 jobs
submitted by \verb@group_physics@ users).  Also, the default priority
...
\begin{verbatim}
  ...
  #
  # Example 2
  # Submit file for group quota user
  #
  ####################
  ...
...
the \verb@group_physics@ group and that the user is newton (remember that
