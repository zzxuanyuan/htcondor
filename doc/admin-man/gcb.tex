...
\begin{itemize}
  ...
  \item Direct outbound connection to a public node: the server will
  see either the private IP address, or the IP of the NAT server for
  the private network (if NAT is being used).

  \item Connection to a host in a different private network that must
  be relayed through the GCB broker: the server will see the public IP
  address of the broker representing the client.
...
\begin{itemize}
  ...
  of a non-GCB aware public client, or a misconfigured or missing GCB
  routing table): the private server will see the public IP address
  its own GCB broker.
