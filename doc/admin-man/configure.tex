...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\label{sec:Condor-wide-Config-File-Entries}Condor-wide
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\label{sec:Daemon-Logging-Config-File-Entries}Daemon
Logging Config File Entries} 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\label{sec:DaemonCore-Config-File-Entries}DaemonCore
Config File Entries} 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\Macro{SUBSYS\_EXPRS}] \label{param:SubsysAttrs}
  ...
  These settings are synonyms; you can use them interchangeably or use both.
  For example, \MacroNI{STARTD\_ATTRS} and
  \MacroNI{STARTD\_EXPRS} are interchangeable.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\label{sec:Network-Related-Config-File-Entries}Network-Related
Configuration File Entries}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\label{sec:Shared-Filesystem-Config-File-Entries}Shared
File System Configuration File Macros} 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\label{Checkpoint-Server-Config-File-Entries}Checkpoint
Server Configuration File Macros} 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\label{sec:Master-Config-File-Entries}\condor{master}
...
\item[\Macro{PREEMPT}] \label{param:Preempt}   A boolean expression
   that, when \Expr{True}, causes Condor to not continue
   running a currently suspended job in favor of running a different one 
   (with higher priority or better rank).
...
\item[\Macro{RunBenchmarks}] \label{param:RunBenchmarks}
  Specifies when to run benchmarks.  When the machine is in the Unclaimed state and this expression evaluates to TRUE, benchmarks will be run.  If RunBenchmarks is specified and set to anything other than FALSE additional benchmarks will be run when the startd initially starts.  To disable startup benchmarks you must set \MacroNI{RunBenchmarks} to FALSE or comment it out of the configuration file.
...
\label{param:StartdCronAutopublish}
   ...
   \begin{description}
     ...
     \Condor{startd} will not automatically publish updates based on
     any cron modules, and will rely on the usual behavior for sending
     updates (periodically, based on the \Macro{UPDATE\_INTERVAL}
     configuration setting, or whenever a given virtual machine
     changes state).
     \item[\texttt{Always}] The \Condor{startd} will always send a new
     update to the \Condor{collector} whenever any cron module exits.
     \item[\texttt{If\_Changed}] The \Condor{startd} will only send a
...
\label{param:StartdCronModuleKill}
    ...
    This macro controls the behaviour of the \Condor{startd} when it
...
HAKWEYE_job1_PREFIX = prefix_
HAKWEYE_job1_EXECUTABLE = $(MODULES)/job1
HAKWEYE_job1_PERIOD = 5m
HAKWEYE_job1_MODE = WaitForExit
HAKWEYE_job1_KILL = false
...
\label{param:StartdCronJobs}
  ...
  Note: This syntax is now depricated, and replaced by the new
  \MacroNI{STARTD\_CRON\_JOBLIST} syntax (see above).
  For more information on the Cron logic, read the above discussion.
  If \MacroNI{STARTD\_CRON\_JOBLIST} is defined,
  \MacroNI{STARTD\_CRON\_JOBS} is ignored.
...
\item[\Macro{JOB\_IS\_FINISHED\_INTERVAL}] \label{param:JobIsFinishedInterval}
  ...
  tells the schedd to not impose any delay.  
...
\item[\Macro{ALIVE\_INTERVAL}] \label{param:AliveInterval} This
  ...
  If the startd does not receive any of these keep alive messages
  ...
  the startd releases the claim, and the schedd no longer pays for
...
\item[\Macro{GROUP\_PRIO\_FACTOR\_<groupname>}] \label{param:GroupPrioFactorGroupname}
  ...
  specifies that all member of the group named \verb@group_physics@
...
should probe the remote schedd to check the jobs status.  
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\label{sec:GridMonitor-Config-File-Entries}\Prog{grid\_monitor}
Configuration File Entries}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\item[\Macro{MASTER\_<name>\_CONTROLLER}]
  ...
  with the \Condor{had} daemon serving as the controller process.
  ...
  As of 6.7.13, this should be specified for all configurations using
  \Condor{had}.   To configurate the \Condor{negotiator}
  to controlled by \Condor{had}:
...
\item[\Macro{QUILL\_<NAME>}] \label{param:QuillName}
  ...
  daemon.
  A convient name to choose is of the form
\footnotesize
\begin{verbatim}
