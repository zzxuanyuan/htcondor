...
\item[\MacroB{<SUBSYS>\_LEVEL\_LOG}] \label{param:SubsysLevelLog}
\index{SUBSYS\_LEVEL\_LOG macro@\texttt{<SUBSYS>\_LEVEL\_LOG} macro}
  This is
...
\item[\Macro{MAX\_<SUBSYS>\_LEVEL\_LOG}] \label{param:MaxSubsysLevelLog}
  Similar to \Macro{MAX\_<SUBSYS>\_LOG}.

\item[\Macro{TRUNC\_<SUBSYS>\_LEVEL\_LOG\_ON\_OPEN}]
...
\item[\Macro{ENABLE\_RUNTIME\_CONFIG}]
  ...
  enable it by setting \MacroNI{ENABLE\_RUNTIME\_CONFIG} to TRUE, and
...
\item[\Macro{ENABLE\_PERSISTENT\_CONFIG}]
  ...
  default, Condor daemons will not honor these requests.
  To use this functionality, Condor administrators must specifically
  enable it by setting \MacroNI{ENABLE\_PERSISTENT\_CONFIG} to TRUE,
...
\item[\Macro{PERSISTENT\_CONFIG\_DIR}]
  ...
  A directory (only writable by root) on the local filesystem is
...
\label{param:StartdCronConfigVal}
  ...
  \MacroNI{\MacroUNI{STARTD\_CRON\_NAME}\_CONFIG\_VAL}.  Exmaple:
...
\item[\Macro{SCHEDD\_ROUND\_ATTR\_<xxxx>}]
	\label{param:ScheddRoundAttr} This macro instructs the schedd to round up 
	scalar job ClassAd attributes.  This is desirable to improve job
	scheduling performance, since it allows Condor's auto-clustering algorithm
	to ``bucket" attributes like \AdAttr{ImageSize}.  
	Replace ``<xxxx>" with the name of the attribute to round, and set this
	macro equal to the number of decimals to round up.  For example, to
	round the value of job ClassAd attribute \Attr{foo}  up to the nearest
	100, set 
\begin{verbatim}
        SCHEDD_ROUND_ATTR_foo = 3
\end{verbatim}
...
\end{verbatim}
	Thus, the above attributes are all rounded up from kilobytes 
	to the nearest 10 megabytes by default.
...
\item[\Macro{PREEMPTION\_REQUIREMENTS}]
  ...
  ``churning'' of jobs in the pool.  Note that this setting does not
...
\item[\Macro{ENABLE\_GRID\_MONITOR}] \label{param:EnableGridMonitor}
  ...
  This parameter only affects grid-type gt2 grid universe jobs.
...
\item[\Macro{GRID\_MONITOR}] \label{param:GridMonitor}
  ...
  Globus gatekeepers.  This parameter only affects grid-type gt2 grid
...
%   section~\ref{sec:Condor-Subsystem-Names}.
% 
% \item[\MacroB{<SUBSYS>\_SOAP\_SSL\_SERVER\_KEYFILE}]
%   \label{param:SubsysSoapSSLServerKeyfile}
%   \index{SUBSYS\_SOAP\_SSL\_SERVER\_KEYFILE macro@\texttt{<SUBSYS>\_SOAP\_SSL\_SERVER\_KEYFILE} macro}
...
%   section~\ref{sec:Condor-Subsystem-Names}.
% 
% \item[\MacroB{<SUBSYS>\_SOAP\_SSL\_SERVER\_KEYFILE\_PASSWORD}]
%   \label{param:SubsysSoapSSLServerKeyfilePassword}
%   \index{SUBSYS\_SOAP\_SSL\_SERVER\_KEYFILE\_PASSWORD macro@\texttt{<SUBSYS>\_SOAP\_SSL\_SERVER\_KEYFILE\_PASSWORD} macro}
...
%   section~\ref{sec:Condor-Subsystem-Names}.
% 
% \item[\MacroB{<SUBSYS>\_SOAP\_SSL\_CA\_FILE}]
%   \label{param:SubsysSoapSSLCaFile}
%   \index{SUBSYS\_SOAP\_SSL\_CA\_FILE macro@\texttt{<SUBSYS>\_SOAP\_SSL\_CA\_FILE} macro}
...
%   section~\ref{sec:Condor-Subsystem-Names}.
% 
% \item[\MacroB{<SUBSYS>\_SOAP\_SSL\_CA\_DIR}]
%   \label{param:SubsysSoapSSLCaDir}
%   \index{SUBSYS\_SOAP\_SSL\_CA\_DIR macro@\texttt{<SUBSYS>\_SOAP\_SSL\_CA\_DIR} macro}
...
%   section~\ref{sec:Condor-Subsystem-Names}.
% 
% \item[\MacroB{<SUBSYS>\_SOAP\_SSL\_DH\_FILE}]
%   \label{param:SubsysSoapSSLDhFile}
%   \index{SUBSYS\_SOAP\_SSL\_DH\_FILE macro@\texttt{<SUBSYS>\_SOAP\_SSL\_DH\_FILE} macro}
