...
\item[\Macro{RunBenchmarks}] \label{param:RunBenchmarks}
  Specifies when to run benchmarks.  When the machine is in the Unclaimed state and this expression evaluates to TRUE, benchmarks will be run.  If RunBenchmarks is specified and set to anything other than FALSE additional benchmarks will be run when the \Condor{startd} initially starts.  To disable startup benchmarks you must set \MacroNI{RunBenchmarks} to FALSE or comment it out of the configuration file.
...
\item[\Macro{ALIVE\_INTERVAL}] \label{param:AliveInterval} This
  ...
  When the schedd claims a \Condor{startd}, it tells the \Condor{startd} how often it is
  ...
  the \Condor{startd} releases the claim, and the schedd no longer pays for
