...
\item[\Macro{MASTER\_HA\_LIST}] \label{param:MasterHAList} Similar to
  \Macro{DAEMON\_LIST}, this macro determines what daemons the
  \Condor{master} will start and keep its watchful eyes on.  However,
  the \Macro{MASTER\_HA\_LIST} daemons are run in a ``High
  Availability'' mode.  The list
  is a comma or space separated list of subsystem names (listed in 
  section~\ref{sec:Condor-Subsystem-Names}).  For example,
\begin{verbatim}
        MASTER_HA_LIST = SCHEDD
\end{verbatim}

  The ``High Availability'' feature allows for several \Condor{master}
  ...
  To use this feature, the lock URL will need to be set with
  \Macro{HA\_LOCK\_URL}.

  Currently, only file URLs are supported ``file://''.  The default
  value for \Macro{MASTER\_HA\_LIST} is ``'', which disables this
  feature.
  
\item[\Macro{HA\_LOCK\_URL}] \label{param:HALockURL} This macro
  specifies the URL that the \Condor{master} processes use to
  syncronize for the ``High Availability'' service (see
  \Macro{MASTER\_HA\_LIST}).  Currently, only file URLs are supported,
  such as``file://share/spool''.  Note that this URL must be identical
  for all \Condor{master} processes sharing this resource.  For
  \Condor{schedd} sharing, we recomend setting up the \Macro{SPOOL} on
  an NFS share and having all HA \Condor{schedd} processes sharing it,
  and setting the \Macro{HA\_LOCK\_URL} to point at this directory, as
  well.  For example:
\begin{verbatim}
        MASTER_HA_LIST = SCHEDD
        SPOOL = /share/spool
...
\end{verbatim}
  ...
  There is no default value for \Macro{HA\_LOCK\_URL}.

\item[\Macro{HA\_daemon\_LOCK\_URL}]
  \label{param:HAdaemonLockURL} This macro controls the ``High
  Availability'' lock URL for a specific daemon ``daemon'' and
  overrides the system wide lock URL specified by
  \Macro{HA\_LOCK\_URL}.  If not defined for each daemon,
  \Macro{HA\_daemon\_LOCK\_URL} is ignored, and the value of
  \Macro{HA\_LOCK\_URL} is used.

\item[\Macro{HA\_LOCK\_HOLD\_TIME}] \label{param:HALockHoldTime} This macro
  specifies the length of time that the \Condor{master} will hold the
  lock for each ``High Availability'' daemon.  Upon gaining the shared
  lock, the \Condor{master} will hold the lock for this number of
  seconds. Additionally, the \Condor{master} will periodically renew
  ...
  \Macro{HA\_LOCK\_HOLD\_TIME} defaults to 3600 seconds (one hour).

\item[\Macro{HA\_daemon\_LOCK\_HOLD\_TIME}]
  \label{param:HAdaemonLockHoldTime} This macro controls the ``High
  Availability'' lock hold time for a specific daemon ``daemon'' and
  overrides the system wide poll period specified by
  \Macro{HA\_LOCK\_HOLD\_TIME}.  If not defined for each daemon,
  \Macro{HA\_daemon\_LOCK\_HOLD\_TIME} is ignored, and the value of
  \Macro{HA\_LOCK\_HOLD\_TIME} is used.

\item[\Macro{HA\_POLL\_PERIOD}] \label{param:HALockPollPeriod} This
  macro specifies how often the \Condor{master} will poll the ``High
  Availability'' locks to see if any locks are either stale (not
  updated for \Macro{HA\_LOCK\_HOLD\_TIME} seconds), or have been
  released by the owning \Condor{master}.  Addionally, the
  \Condor{master} will renew any locks that it ``owns'' during these
  polls.

  \Macro{HA\_POLL\_PERIOD} defaults to 300 seconds (five minutes).

\item[\Macro{HA\_daemon\_POLL\_PERIOD}]
  \label{param:HALockPolldaemonPeriod} This macro controls the ``High
  Availability'' pool period for a specific daemon ``daemon'' and
  overrides the system wide poll period specified by
  \Macro{HA\_POLL\_PERIOD}.  If not defined for each daemon,
  \Macro{HA\_daemon\_POLL\_PERIOD} is ignored, and the value of
  \Macro{HA\_POLL\_PERIOD} is used.
...
\end{verbatim}
  ...
  These individual fields are described below: \begin{itemize}
    ...
    For ``Wait For Exit'' mode, the value has a different meaning; in this case
   ...
   the last one in the list is followed.
	...
	\item The ``WaitForExit'' option is used to specify a module
	which runs in the ``Wait For Exit'' mode (as described above).
	In ``Wait For Exit'' mode, the \Condor{startd} daemon
	...
	\item The ``ReConfig'' option is used to specify that a module
	can handle HUP signals, and should be sent a HUP signal when
	the \Condor{startd} daemon is reconfigured.  The module is
	expected to reread it's configuration at that time.

	Note that the HUP signal will never be delivered to a module
	until it produces it's first block of ``publishable'' data
	(i.e. the module exits or produces a ``-'' alone on a line).

	\item The ``NoReConfig'' option is used turn off the
	``reconfig'' option.  This is the default (except for modules
	using the deprecated ``continuous'' mode). 

	\item The ``Kill'' option is for
	for a periodic mode of module execution,
	...
	This option has no effect on ``Continuous'' or ``WaitForExit'' jobs.

	\item The ``NoKill'' option is
	for a periodic mode of module execution,
	the module may still be running when the period is up
	and it is time to start the module again.
	This option allows the module to continue execution.
	This is the default for noncontinuous mode.
	This option has no effect on ``Continuous'' or ``WaitForExit''
	jobs.
    \end{itemize}
...
\label{param:StartdAvailConfidence}
  ...
  startd daemon's AvailTime estimate.
...
\item[\Macro{JOB\_START\_COUNT}] \label{param:JobStartCount}
  ...
  (below) to pace job starts.  See the \Macro{JOB\_START\_DELAY} macro
  below.  The default and minimum values for the \Macro{JOB\_START\_COUNT}
  macro are both 1.  Small values for this macro are preferred and will
  minimize the load upon the \Condor{schedd}.
  
\item[\Macro{JOB\_START\_DELAY}] \label{param:JobStartDelay}
  This macro works together with the \Macro{JOB\_START\_COUNT} macro
  (above) to pace job starts.  The  \Condor{schedd} will start
  \Macro{JOB\_START\_COUNT} jobs as quickly as possible, then delay for
  \Macro{JOB\_START\_DELAY} seconds, before starting the next set of
  jobs.  The interval "as quickly as possible" indicates the time
  required for the \Condor{schedd} to claim \Macro{JOB\_START\_COUNT}
  machines, and can be as small as zero.
  This delay prevents a sudden, large load on the submit
  machine as it spawns many shadows simultaneously, and prevents
  having to deal
  with their startup activity all at once.
  The resulting job start rate
  can average as fast as
  (\Macro{JOB\_START\_COUNT}/\Macro{JOB\_START\_DELAY}) jobs/second.
  ...
  Defined in terms of seconds and defaults to 2.  Setting this macro to
  a lower value is not advised, and can overwhelm the \Condor{schedd}.
...
\item[\Macro{REQUEST\_CLAIM\_TIMEOUT}]
  ...
  seconds) that the schedd will wait for a claim to be granted by the
  startd.  The default is 30 minutes.  This is only likely to matter
  if the startd has an existing claim and it takes a long time for the
  existing claim to be preempted due to \Expr{MaxJobRetirementTime}.
  Once a request times out, the schedd will simply begin the process
...
\label{param:MyProxyGetDelegation}  The full path name
to the \Prog{myproxy-get-delegation} executable,
installed as part of \Prog{MyProxy} software.
Often, it is necessary to wrap the actual executable with a
script that sets the environment,
such as the \MacroNI{LD\_LIBRARY\_PATH} correctly.
