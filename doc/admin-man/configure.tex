...
You can tell Condor to run a program to obtain configuration parameters
instead of reading a file. If a configuation filename you provide 
(\Env{CONDOR\_CONFIG} environment variable, \Macro{LOCAL\_CONFIG\_FILE}
macro, or \Macro{LOCAL\_ROOT\_CONFIG\_FILE} macro) ends with a vertical
bar (``\Bar''), it is executed and its standard output is parsed for
configuration parameters. If \Macro{LOCAL\_CONFIG\_FILE} or
\Macro{LOCAL\_ROOT\_CONFIG\_FILE} is used in this way, then it can only
contain a single item, and spaces in the value will be interpreted as
part of the command to be executed (separating the executable and any
...
\item[\Macro{LOCAL\_CONFIG\_FILE}] \label{param:LocalConfigFile} The
  ...
  to prevent cycles.
  If you use the vertical bar notation described in 
  section~\ref{sec:Intro-to-Config-Files} on 
  page~\pageref{sec:Intro-to-Config-Files}, \Macro{LOCAL\_CONFIG\_FILE}
  is treated as a single command to be executed.
...
\item[\Macro{NET\_REMAP\_ENABLE}] 
  If defined to \Expr{True}, this setting will enable a network
