...
\item[\Macro{CLAIM\_WORKLIFE}] \label{param:ClaimWorklife}
  ...
  a \Condor{schedd} daemon's claim to a \Condor{startd} daemon
  will enter retirement.  In retirement,
  any existing job may continue to run (subject to other preemption policy
  expressions), but no new jobs may be started.  This forces the 
  \Condor{schedd} daemon
  to go back to the \Condor{negotiator} daemon to request a new claim,
  rather than
