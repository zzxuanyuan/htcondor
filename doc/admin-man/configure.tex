...
is used, except for the collector and the schedd, which default to 10240 and
4096, respectively.
...
The following variables are for the configuration of Consumption Policies
for slots advertised by the \Condor{startd} daemon.

\begin{description}

\label{param:ConsumptionPolicy}
\item[\Macro{CONSUMPTION\_POLICY}]
  A boolean configuration variable that defaults to \Expr{False}.
  When \Expr{True}, enables consumption policies for partitionable
  slots on the \Condor{startd} daemon.  May be overridden on a 
  per-slot-type basis using SLOT\_TYPE\_<N>\_CONSUMPTION\_POLICY.

\label{param:ConsumptionResource}
\item[\Macro{CONSUMPTION\_<Resource>}]
  Specifies a consumption policy for a particular resource partitionable
  slot.  The following example configures a consumption policy that rounds
...
\end{verbatim}
  ...
  slot must have such a policy configured, including any extensible
  resources if they are configured.  Each resource consumption policy
  may be overridden on a per-slot-type basis using
  SLOT\_TYPE\_<N>\_CONSUMPTION\_<Resource>.  The following resources
  are always advertised by \Condor{startd}, and therefore have
  default values:
\begin{verbatim}
# Consumption policies for resources with default values:
CONSUMPTION_CPUS = quantize(target.RequestCpus,{1})
CONSUMPTION_MEMORY = quantize(target.RequestMemory,{128})
CONSUMPTION_DISK = quantize(target.RequestDisk,{1024})
\end{verbatim}
  Note that extensible resources have no default consumption policy.

\label{param:SlotWeight}
\item[\Macro{SLOT\_WEIGHT}]
  Specifies an expression that evaluates to a slot's weight, as used by
  the \Condor{negotiator} daemon during matchmaking.
  Defaults to \Expr{Cpus}.  In this example, a memory-centric slot
...
\end{verbatim}
  ...
  Slot weights may be overridden on a per-slot-type basis using
  SLOT\_TYPE\_<N>\_SLOT\_WEIGHT.

\label{param:NumClaims}
\item[\Macro{NUM\_CLAIMS}]
  Specifies the number of claims a partitionable slot will advertise for use
  by the \Condor{negotiator} daemon.  In the case of slots advertising a
  consumption policy, the negotiator may match more than one job to the slot
  in a single negotiation cycle.  For partitionable slots with a consumption
  policy, NUM\_CLAIMS defaults to the number of cpus owned by the slot.  Otherwise,
  it defaults to 1.
...
\item[\Macro{FILE\_TRANSFER\_DISK\_LOAD\_THROTTLE}]
  ...
  single floating point number or as a range.  Example: \Expr{5 to
  6.5}.  If only a single number is provided, this serves as the
...
\item[\Macro{FILE\_TRANSFER\_DISK\_LOAD\_THROTTLE\_WAIT\_BETWEEN\_INCREMENTS}]
  ...
  increments to the concurrency level set by \Macro{FILE\_TRANSFER\_DISK\_LOAD\_THROTTLE}.
...
\item[\Macro{FILE\_TRANSFER\_DISK\_LOAD\_THROTTLE\_SHORT\_HORIZON}]
  This rarely configured variable specifies the name of the short
  monitoring timespan to use for throttling.  The named timespan must
...
\item[\Macro{FILE\_TRANSFER\_DISK\_LOAD\_THROTTLE\_LONG\_HORIZON}]
  This rarely configured variable specifies the name of the long
  monitoring timespan to use for throttling.  The named timespan must
  exist in \Macro{TRANSFER\_IO\_REPORT\_TIMESPANS}.  The default is
  \Expr{"5m"}.
...
\item[\Macro{VALID\_SPOOL\_FILES}]
  ...
  A single wildcard is permitted in each entry.
...
The path the the \Condor{dagman\_metrics\_reporter} executable.  Defaults to
\Macro{\$(LIBEXEC)/condor\_dagman\_metrics\_reporter}.

\label{param:DAGmanPegasusReportTimeout}
\item[\Macro{DAGMAN\_PEGASUS\_REPORT\_TIMEOUT}]
This integer value defaults to 100 seconds.  The
\Condor{dagman\_metrics\_reporter} will exit after successfully contacting all
its metrics servers or after it has executed for more than this duration of
time.
\end{description}
...
\item[\Macro{GANGLIAD\_VERBOSITY}] An integer that specifies the
  ...
  not published to the collector when using the default value of
...
\item[\Macro{GANGLIAD\_REQUIREMENTS}] An optional ClassAd expression
  that restricts the set of daemon ads to be monitored.  This could be
  ...
  monitored ads.  Keep in mind that this expression is applied to
  all types of monitored ads, not just startd ads.
...
\item[\Macro{GANGLIA\_CONFIG}]
  Path to the Ganglia configuration file.  The default is \File{/etc/ganglia/gmond.conf}.

\label{param:GangliaGmetric}
\item[\Macro{GANGLIA\_GMETRIC}]
  Full path of the \Prog{gmetric} executable to use.  If none is
  specified, \File{libganglia} will be used instead if possible,
  because the library interface is more efficient than invoking
  \Prog{gmetric}.  Some versions of \File{libganglia} are not
  compatible.  When failure to use \File{libganglia} is detected,
  \Prog{gmetric} will be used if it can be found in HTCondor's
  \Env{PATH}.

\label{param:GangliaLib}
\item[\Macro{GANGLIA\_LIB}]
  Full path of the \File{libganglia} shared library to use.  If none is
  ...
  special value NOOP indicates that \Condor{gangliad} should not
...
\item[\Macro{GANGLIA\_LIB\_PATH}]
  Comma-separated list of directories in which to search for \File{libganglia} if
  \Macro{GANGLIA\_LIB} is not configured.  This is used in 32-bit versions of HTCondor.

\label{param:GangliaLib64Path}
\item[\Macro{GANGLIA\_LIB64\_PATH}]
  Comma-separated list of directories in which to search for \File{libganglia} if
  \Macro{GANGLIA\_LIB} is not configured.  This is used in 64-bit versions of HTCondor.
...
\item[\Macro{GANGLIAD\_DEFAULT\_IP}]
  An expression specifying the default IP of Ganglia metrics.  The
  expression may refer to attributes of the machine.

\label{param:GangliadLog}
\item[\Macro{GANGLIAD\_LOG}] The path to the \Condor{gangliad} daemon's log file.
  The default log location is \File{\$(LOG)/GangliadLog}.
...
\item[\Macro{GANGLIAD\_METRICS\_CONFIG\_DIR}]
  ...
  of each ad define the properties of a metric.  The value of each
  attribute may be a ClassAd expression that references properties of
  the daemon ad that is being monitored.
...
\end{verbatim}
  ...
  The file containing the default metrics is a good source of
  additional examples: \File{\$(GANGLIAD\_METRICS\_CONFIG\_DIR)/00\_default\_metrics}.
  ...
  \item[Value] A ClassAd expression that produces the value of this
    metric.  The default value is the value in the daemon ad of the
  ...
  \item[TargetType] A string containing a comma-separated list of daemon
    ...
    match \Attr{MyType} of the daemon ad.  In addition, there are
    special values that may be included. \verb|"Machine_slot1"| may be
    specified to monitor the startd ad for slot 1 only.  This is
    useful when monitoring machine-wide attributes.  The special
    value \verb|"ANY"| matches any type of ad.

  \item[Requirements] If specified, this may be used to restrict which
    daemon ads are monitored by this metric.  By default, all daemon
    ads of the specified type are monitored.
  ...
  \item[Regex] This string value specifies a regular expression that
    ...
    not known in advance, a special ClassAd attribute in the daemon ad
    ...
    expression.  These may be substitued into the values of other
  ...
  \item[Aggregate] This string value specifies an aggregation function
    ...
    ad.  Possible values are \verb|"sum"|, \verb|"avg"|, \verb|"max"|,
    and \verb|"min"|.
  ...
  \item[AggregateGroup] When an aggregate function has been specified,
    ...
    collector host.  To group by machine and publish the result into
    ...
    name, rather than the collector's.
  ...
  \item[Machine] The name of the host associated with this metric.  If
    ...
    is taken from the \Attr{Machine} attribute of the daemon ad.  If
    the daemon name is of the form \verb|name@hostname|, this may
    ...
    value of \Attr{Machine} will be the name of the collector host.
  ...
  \item[IP] A string containing the IP address of the host associated
    ...
    attribute of the daemon ad.  This value must be unique for each
