...
They specify items related to the \Condor{vm-gahp}.  Please note that 
this files does not inherit macros from \Condor{config}.  So, for
instance, \File{\MacroUNI{BIN}/condor_vm_vmware.pl} will not work for
\Macro{VM\_GAHP\_LOG}.  Instead, the macro \MacroUNI{BIN} must be 
re-declared, or a full paths must be used in every case.  This applies 
to configurations using Xen as well.
