...
\item[\Macro{WINDOWS\_FIREWALL\_FAILURE\_RETRY}]
  \label{param:WindowsFirewallFailureRetry} When a Windows machine boots
  ...
  Firewall because of a delay in other services starting up. This
  configuration parameter allows administrators to set the number of
  times (once every 10 seconds) that the \Condor{master} will retry
  to add firewall exceptions. Setting this to 0 means that Condor will
  retry indefinitely. The default is 60.
...
\item[\Macro{NEGOTIATOR\_INTERVAL}] \label{param:NegotiatorInterval}
  ...
  The minimum accepted value is 20. Values lower than 20 are treated as 20.
