...
\label{param:StartdCronJobs}
  ...
  \MacroNI{STARTD\_CRON\_JOBLIST} syntax (see above) which should be
  used instead.
  For more information on the Cron logic, read the above discussion.
  Currently, both \MacroNI{STARTD\_CRON\_JOBLIST} and
  \MacroNI{STARTD\_CRON\_JOBS} are procesed, but any conficting
  entries in \MacroNI{STARTD\_CRON\_JOBS} will be ingored.  Future
  versions of Condor may ignore \MacroNI{STARTD\_CRON\_JOBS}.
