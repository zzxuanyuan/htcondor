...
\item[\Macro{MAX\_JOBS\_SUBMITTED}] \label{param:MaxJobsSubmitted}
  This macro limits the number of jobs that can be in a
  \Condor{schedd}'s queue. Any attempted job submissions that would
  exceed this limit will be refused.
...
\item[\Macro{XEN\_DEFAULT\_RAMDISK}]
...
\item[\Macro{QUILL\_USE\_SQL\_LOG}] \label{param:QuillUseSQLLog}
  ...
  \MacroNI{SCHEDD.\_QUIIL\_USE\_SQL\_LOG} to \Expr{True}.
...
\item[\Macro{QUILL\_NOT\_RESPONDING\_TIMEOUT}]
  When the \Condor{quill} is processing a very long logfile, it 
  may not be able to communicate with the master. The \Condor{master} may
  decide that this means the \Condor{quill} is hung, and the \Condor{master}
  may kill and restart the \Condor{quill}. 
...
\item[\Macro{DATABASE\_REINDEX\_INTERVAL}] 
  ...
  The default value is 86400, or one day.
  This is only used when the \MacroNI{QUILL\_DB\_TYPE} is set to PGSQL.

\item[\Macro{QUILL\_JOB\_HISTORY\_DURATION}] \label{param:QuillJobHistoryDuration}
...
\item[\Macro{QUILL\_RUN\_HISTORY\_DURATION}] \label{param:QuillRunHistoryDuration}
...
\item[\Macro{QUILL\_RESOURCE\_HISTORY\_DURATION}] \label{param:QuillResourceHistoryDuration}
...
\item[\Macro{DBMSD}] \label{param:DBMSD}  The full path name to the
  \Condor{dbmsd} daemon.
...
\item[\Macro{DBSMD\_NOT\_RESPONDING\_TIMEOUT}]
  When the \Condor{dbmsd} is purging or reindexing a very large database, it 
  may not be able to communicate with the master. The \Condor{master} may
  decide that this means the \Condor{dbmsd} is hung, and the \Condor{master}
  may kill and restart the \Condor{dbmsd}. 
  The default for this setting is 3600 seconds, or one hour. It may be
  advisable to increase this to several hours. 
