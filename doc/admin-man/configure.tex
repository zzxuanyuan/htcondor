...
\item[\Macro{DAEMON\_SHUTDOWN}] \label{param:DaemonShutdown}
  ...
  this expression.
  If it evaluates to TRUE, the daemon will gracefully shut itself down
...
\item[\Macro{STARTER\_ALLOW\_RUNAS\_OWNER}]
  ...
  target) that determins whether the job may run under the job owner's
...
\item[\Macro{PERIODIC\_EXPR\_TIMESLICE}]
  ...
  evaluate these expressions less frequently.  The default timeslice
