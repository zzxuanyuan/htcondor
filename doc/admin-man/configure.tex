...
\item[\Macro{SUBSYS\_EXPRS}] \label{param:SubsysExprs}
  ...
  \MacroU{SUBMIT\_EXPRS}. 

  The \Condor{startd} \MacroU{STARTD\_EXPRS} defaults to
  ``JobUniverse''.
...
\item[\Macro{NUM\_CPUS}] \label{param:NumCpus}
  ...
  \MacroNI{STARTD\_EXPRS} setting (described above).
...
\item[\Macro{SCHEDD\_EXPRS}] \label{param:ScheddExprs} This macro is
  ...
  section~\ref{param:SubsysExprs} on \MacroNI{SUBSYS\_EXPRS}.
...
\item[\Macro{CONDOR\_DEVELOPERS}] \label{param:CondorDevelopers}
  ...
  value of \textit{condor-admin@cs.wisc.edu} will send this report to
  ...
  not wish this information to be sent to the Condor Team, you must explicitly
  set the value to \textit{NONE} to disable this feature, or put in some other
  address that you want the weekly status report sent to.  If this macro
  definition is commented out in the configuration file, Condor will assume
  the default behavior to send the weekly email.
...
\item[\Macro{CONDOR\_DEVELOPERS\_COLLECTOR}]
  ...
  By default, they will be sent to \textit{condor.cs.wisc.edu}.  If you don't want
  these updates to be sent from your pool, you must explicitly set this macro to
   \textit{NONE}. 
  If this macro
  definition is commented out in the configuration file, Condor will assume
  the default behavior to send the periodic updates.
...
\item[\Macro{NEGOTIATOR\_POST\_JOB\_RANK}]
  ...
  are first sorted by the \Macro{NEGOTIATOR\_POST\_JOB\_RANK}
