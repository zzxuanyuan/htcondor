...
\item[\Macro{LOG}] \label{param:Log} Used to specify the
  ...
  be placed
  in the directory defined by this macro.  Normally, \MacroUNI{LOG} is
...
\item[\Macro{SPOOL}] \label{param:Spool} The spool directory is where
  ...
  Normally, \MacroUNI{SPOOL} is defined in terms of
  \MacroUNI{LOCAL\_DIR}.
...
\item[\Macro{EXECUTE}] \label{param:Execute} This directory acts as
  ...
  (or this macro defined).  Normally, \MacroUNI{EXECUTE} is defined
  in terms of \MacroUNI{LOCAL\_DIR}.
...
\end{verbatim}
  ...
  Beginning with Condor version 6.0.1, the \MacroUNI{LOCAL\_CONFIG\_FILE}
...
\item[\Macro{REQUIRE\_LOCAL\_CONFIG\_FILE}] \label{param:RequireLocalConfigFile}
  ...
  file cannot be read, unless \MacroUNI{REQUIRE\_LOCAL\_CONFIG\_FILE} is set 
  to False 
...
\item[\Macro{RESERVED\_DISK}] \label{param:ReservedDisk} Determines
  ...
  free space in the \MacroUNI{EXECUTE} directory.
...
\item[\Macro{Q\_QUERY\_TIMEOUT}] \label{param:QQueryTimeout}
  ...
  connect to the \Condor{schedd}.  Defaults to 20.
...
\item[\Macro{UPDATE\_COLLECTOR\_WITH\_TCP}]
  ...
  functionality. 
  At this time, this setting only effects the main \Condor{collector}
  for your site, not any sites that a \Condor{schedd} might flock to. 
  If you enable this feature, you must also define
  \Macro{COLLECTOR\_SOCKET\_CACHE\_SIZE}] at your central manager, so
...
The name of an imaginary user for Condor to use instead of
user nobody,
...
is incorporated into the machine ClassAd generated by the \Condor{startd}.
...
\item[\Macro{COLLECTOR\_DAEMON\_HISTORY\_SIZE}]
  ...
  macro is ignored if \MacroU{COLLECTOR\_DAEMON\_STATS} is not
  enabled.
...
\item[\Macro{NEGOTIATOR\_PORT}] \label{param:NegotiatorPort}
  The port that the \Condor{negotiator} uses. Defaults to 9614.
