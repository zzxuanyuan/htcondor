...
\item[\Macro{CLAIM\_WORKLIFE}] \label{param:ClaimWorklife}
  An integer value representing the number of seconds after which
  a schedd's claim to a startd will enter retirement.  In retirement,
  any existing job may continue to run (subject to other preemption policy
  expressions), but no new jobs may be started.  This forces the schedd
  to go back to the negotiator to request a new claim, rather than
  keeping resource claims indefinitely while it has jobs to run on them.
  The default value of -1 is treated as an infinite claim worklife, so
  claims may be held indefinitely.
...
\item[\Macro{NEWLY\_JOINED\_WAITING\_VERSION\_INTERVAL}]
  \label{param:NewlyJoinedWaitingVersionInterval}
