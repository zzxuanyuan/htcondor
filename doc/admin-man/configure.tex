...
\label{param:DeadCollectorMaxAvoidanceTime} Defines the maximum time
(in seconds) that a daemon (such as the negotiator) will fail over from
a primary collector to a secondary collector (if any).  The default is
one hour.  If connections to the dead primary collector take very
little time to fail, new attempts to query the primary collector may
...
\label{param:AllowVMCruft}
  A boolean value that defaults to \Expr{True}.  When \Expr{True},
  Condor looks for configuration variables named with the
  previously used string \MacroNI{VM} instead of the currently
  used string \MacroNI{SLOT}.
