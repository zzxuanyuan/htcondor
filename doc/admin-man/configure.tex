...
\item[\Macro{SCHEDD\_CLUSTER\_MAXIMUM\_VALUE}]
  ...
  continue assignment using \Macro{SCHEDD\_CLUSTER\_INITIAL\_VALUE}. The 
...
\item[\Macro{AMAZON\_GAHP}]
  ...
  There is no default value. The expected location as given
  in the example configuration is \File{\MacroUNI{SBIN}/amazon\_gahp}.
...
\item[\Macro{DAGMAN\_OLD\_RESCUE}]
  A boolean value that controls whether \Condor{dagman} uses 
  Rescue DAG naming as defined in Condor versions from before 7.1.0 
  when creating a Rescue DAG.
  When \Expr{True}, the older style naming is used.
  In this older style Rescue DAG file naming,
  if a DAG input file is named \File{my.dag}, the rescue DAG file will
  be \File{my.dag.rescue}, and that file will be overwritten if the re-run
  \File{my.dag} fails again.  
  With the current Rescue DAG file naming,
