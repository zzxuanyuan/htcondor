...
communicate with a given daemon is to query the \Condor{collector} for
...
However, to be able to query the \Condor{collector}, every Condor tool
or daemon must know the port where it is listening.
The \Condor{collector} is the only daemon with a well-known, fixed
port.
...
running on the same host, each Condor daemon can be configure to write
out its IP and port into a well known file.
This is controlled using the \Macro{SUBSYS\_ADDRESS\_FILE} settings,
...
6.7.5, the \Condor{negotiator} also listened on a fixed, well-defined
...
\Condor{collector} which includes the dynamically assigned port it is
listening on.
...
\Condor{collector} for the classad.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{\label{sec:Ports-NonStandard}Using 
a Non Standard Port for the \Condor{collector}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
