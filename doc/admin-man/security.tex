...
%used to run jobs? 
The Condor system 
prefers that Condor daemons are run as the user \Login{root},
while other common operations are to be owned by
a separate user.
This is usually the
\Login{condor} user, but any user can be chosen. This topic is discussed
...
Condor, however, does not turn on these
...
example.com domain,
but administrator commands can only be run from
the machine command.example.com.
But because host-based security does not distinguish users,
anyone that has access to the machine
command.example.com may execute administrator commands.

Condor is also susceptible to attacks that can impersonate IP addresses.
...
The \Condor{submit} command communicates with the \Condor{schedd}.
The \Condor{config\_val} command may communicate with any Condor daemon.

Behind the scenes, Condor daemons also communicate with each other.
For instance, the \Condor{negotiator} daemon will act as a client,
contacting the \Condor{schedd} daemon to initiate matchmaking.
When a match has been found,
the \Condor{schedd} daemon will act as a client and
contact the \Condor{startd} daemon. 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{\label{sec:security-negotiation} Security Negotiation}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\index{security!negotiation}
...
If the daemon determines that authentication is required,
...
\end{verbatim}
where the \verb@<access-level>@ is one of of the access levels defined above: 
\begin{verbatim}
    DEFAULT
    READ
    WRITE
...
Example 1: A user submits a job with \Condor{submit}, and this
...
Example 3: If a user executes \Condor{status} on a machine
where the configuration specifies
\begin{verbatim}
SEC_DEFAULT_AUTHENTICATION = NEVER
\end{verbatim}
and the \Condor{collector}'s configuration specifies
\begin{verbatim}
SEC_READ_AUTHENTICATION = REQUIRED
\end{verbatim}
then \Condor{status} will be
unable to access the information from the \Condor{collector} daemon.
...
place.  Password authentication is available on both UNIX and
Windows. It currently can only be used for daemon-to-daemon
authentication.  The shared secret in this context is referred to as
the ``pool password''.

Before a daemon can use password authentication, the shared secret
must be stored on the daemon's local machine.  On UNIX, the password
will be placed in a file given by the configuration parameter
...
\Condor{master} must be up and running for this command to work.
...
only recommended if it can be done over an encrypted channel.  This is
...
whatever reason (typically because common accounts don't exist or
...
\verb@ALLOW_DAEMON@ and \verb@ALLOW_NEGOTIATOR@ authorization settings
...
\end{verbatim}
This configuration will only allow remote \verb@DAEMON@-level and
