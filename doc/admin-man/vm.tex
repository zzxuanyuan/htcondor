%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{\label{sec:vm-install}VM Universe}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Condor's configuration file, \Condor{config}, includes several of the 
...
You are also required to specify the location of \Condor{vm-gaph}, as well as 
its configuration file. A basic \Condor{vm-gaph} configuration is shipped 
with Condor and can be copied to reside alongside \Condor{config}. For a 
...
\Condor{vm-gaph} should write its logs. By default, this is set to 
\Code{/dev/null} on UNIX and Linux and \Code{NUL} on Windows; however, if 
...
You must also tell the \Condor{vm-gaph} which version of the software you 
are using:
...
\Condor{vm-gaph}. Instead, it is added to the ClassAd for the machine, so it 
...
are eligible to execute the \Condor{vm-gaph}:
...
\Condor{vm-gaph} is installed with setuid-root.  The defaults is initially 
...
you will not need a full path, as \Code{ /usr/sbin} is in the root user's 
...
and \Prog{xenblk}.  This can be easily fixed by creating a new \Code{initrd}
