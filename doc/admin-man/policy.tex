...
\MacroIndex{MaxJobRetirementTime}
\MacroIndex{NEGOTIATOR\_CONSIDER\_PREEMPTION}
\MacroIndex{CLAIM\_WORKLIFE}
\MacroIndex{PREEMPT}
\MacroIndex{PREEMPTION\_REQUIREMENTS}
\MacroIndex{RANK}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
of \Macro{MaxJobRetirementTime}, but this may only cause \emph{less}
...
Note that the use of \Macro{MaxJobRetirementTime} limits the killing
...
the preemption of resource claims.  However, you can achieve the latter
...
#Disable preemption by machine RANK by
#ranking all jobs equally.
RANK = 0
...
If you use this policy, you should be aware of the consequences.
Without any preemption of resource claims, once the negotiator gives
the schedd a match to a machine, the schedd may hold onto this claim
indefinitely, as long as the user keeps supplying more jobs to run.
If that is not what you want, you can force claims to be retired after
some amount of time using \Macro{CLAIM\_WORKLIFE}.  This behaves just
...
job to complete but will not allow any new jobs to be started.  If
there is more work to do, the schedd will therefore have to go back to
the negotiator to request a new match.
