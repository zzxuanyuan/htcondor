...
One machine should run the \Condor{dbmsd}.  On this machine, add it to
the DAEMON\_LIST.  All quill-enabled machines should also run 
the \Condor{quilld}.  The machine running the \condor{dbmsd} can also 
run a \Condor{quilld}.  An example DAEMON\_LIST for a machine running
both daemons, a schedd and a central manager might look like the following:

\begin{verbatim}
DAEMON_LIST                    = MASTER, SCHEDD, COLLECTOR, NEGOTIATOR, DBMSD, QUILL
\end{verbatim}

The DBMSD daemon will need configuration file entries common to all daemons.
If not already in the config file, add the following entries:
