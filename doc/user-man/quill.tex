...
\MacroNI{DATABASE\_NAME} in the \File{condor\_config} file, the quill
...
\MacroNI{SCHEDD\_AD} to the collector, and as such, from the perspective
...
This can be done by simply letting all the \MacroNI{DATABASE\_IPADDRESS}
point to the same database server.

\Notice{If more than one quill server are sharing the same database
server, then the \MacroNI{DATABASE\_NAME} variable for all of them should
...
jobs beyond \MacroNI{QUILL\_HISTORY\_DURATION} will be deleted.  Now,
...
the password that is assigned to database user \Username{quillreader} in
step 2c) above. This variable is then advertised by the quill daemon to
the collector.  This facility enables remote querying: remote \Condor{q}
...
\MacroNI{QUILL\_NAME} variable in part 3) above.  Also, the ip address
and port of the database server hosting the data of this particular remote
quill daemon can be figured out by the \MacroNI{DATABASE\_IPADDRESS} and
\MacroNI{DATABASE\_NAME} variables specified in the \MacroNI{SCHEDD\_AD}
...
\Attr{IsRemotelyQueryable} attribute in the ``scheduler ad'' advertised
