...
\Macro{C\_GAHP\_LOG} setting must point to a location the end
user can write to.
...
that the local schedd reports to.  The remote\_pool
submit file setting can be used to specify the collector 
to contact for the location.
...
This is the most trivial authentication, CLAIMTOBE, which
essentially assigns trust to hostnames or IP addresses.
...
Its value is the same as the schedd ClassAd attribute
...
the job's ClassAd.  You need to specify attributes as they 
should appear in the job's ClassAd, not the submit this. 
Thus,
the \Opt{universe} is specified using an integer assigned for
a job ClassAd \Attr{JobUniverse}.
Similarly, you will need to place quotation marks around string 
expressions.  Where in a normal submit file you might put 
"when\_to\_transfer\_output=ON\_EXIT" to set the value remotely
you will need to specify
"+Remote\_WhenToTransferOutput="ON\_EXIT"".  Note the quotation marks 
around the setting.
